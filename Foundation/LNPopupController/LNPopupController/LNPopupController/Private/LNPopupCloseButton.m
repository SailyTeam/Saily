...
static NSString *const _aPVFGR =
    @"X2FjdGluZ1BhcmVudFZpZXdGb3JHZXN0dXJlUmVjb2duaXplcnM=";

+ (void)load {
  @autoreleasepool {
    Method m = class_getInstanceMethod(self, @selector(_aPVFGR));
    class_addMethod(self,
                    NSSelectorFromString(_LNPopupDecodeBase64String(_aPVFGR)),
...
- (instancetype)initWithContainingContentView:
  ...
  if (self) {
    ...
    if (@available(iOS 13.4, *)) {
      ...
      self.pointerStyleProvider =
        ...
        if (rectValue == nil) {
          return [UIPointerStyle styleWithEffect:proposedEffect
                                           shape:proposedShape];
        }
        ...
        return [UIPointerStyle
            styleWithEffect:proposedEffect
                      shape:[UIPointerShape shapeWithRoundedRect:rect]];
      };
    }
...
- (void)setStyle:(LNPopupCloseButtonStyle)style {
  // This will take care of cases where the user sets
  // LNPopupCloseButtonStyleDefault as well as close button repositioning.
...
- (void)_setupForCircularButton {
  ...
  _effectView = [[UIVisualEffectView alloc]
      initWithEffect:[UIBlurEffect effectWithStyle:blurStyle]];
  _effectView.userInteractionEnabled = NO;
  [self addSubview:_effectView];

  UIVisualEffectView *highlightEffectView = [[UIVisualEffectView alloc]
      initWithEffect:[UIVibrancyEffect
                         effectForBlurEffect:(UIBlurEffect *)
                                                 _effectView.effect]];
  highlightEffectView.autoresizingMask =
      UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
  highlightEffectView.frame = _effectView.contentView.bounds;
  _highlightView =
      [[UIView alloc] initWithFrame:highlightEffectView.contentView.bounds];
  ...
  [self addTarget:self
                action:@selector(_didTouchDown)
      forControlEvents:UIControlEventTouchDown];
  [self addTarget:self
                action:@selector(_didTouchDragExit)
      forControlEvents:UIControlEventTouchDragExit];
  [self addTarget:self
                action:@selector(_didTouchDragEnter)
      forControlEvents:UIControlEventTouchDragEnter];
  [self addTarget:self
                action:@selector(_didTouchUp)
      forControlEvents:UIControlEventTouchUpInside];
  [self addTarget:self
                action:@selector(_didTouchUp)
      forControlEvents:UIControlEventTouchUpOutside];
  [self addTarget:self
                action:@selector(_didTouchCancel)
...
- (void)_setHighlighted:(BOOL)highlighted animated:(BOOL)animated {
  ...
  if (animated) {
    [UIView animateWithDuration:0.47
                          delay:0.0
...
- (void)layoutSubviews {
  ...
  CGPathRef path = CGPathCreateWithRoundedRect(self.bounds, minSideSize / 2,
                                               minSideSize / 2, NULL);
  maskLayer.path = path;
  CGPathRelease(path);
