...
+ (void)load {
  @autoreleasepool {
    ...
    class_addMethod(self, NSSelectorFromString(_LNPopupDecodeBase64String(aCC)),
...
- (void)encodeWithCoder:(NSCoder *)coder {
  if (coder.allowsKeyedCoding == NO) {
    [NSException raise:NSInvalidArgumentException
                format:@"Only coders with allowsKeyedCoding=true supported."];
  }

  [super encodeWithCoder:coder];

  [coder encodeObject:self.titleTextAttributes forKey:@"titleTextAttributes"];
  [coder encodeObject:self.subtitleTextAttributes
               forKey:@"subtitleTextAttributes"];
  ...
  [coder encodeBool:self.coordinateMarqueeScroll
             forKey:@"coordinateMarqueeScroll"];
...
- (instancetype)initWithCoder:(NSCoder *)coder {
  ...
  if (self) {
    self.titleTextAttributes =
        [coder decodeObjectForKey:@"titleTextAttributes"];
    self.subtitleTextAttributes =
        [coder decodeObjectForKey:@"subtitleTextAttributes"];
    self.buttonAppearance = [coder decodeObjectForKey:@"buttonAppearance"];
    self.doneButtonAppearance =
        [coder decodeObjectForKey:@"doneButtonAppearance"];
    self.marqueeScrollEnabled =
        [coder decodeBoolForKey:@"marqueeScrollEnabled"];
    self.marqueeScrollRate =
        [coder decodeDoubleForKey:@"coordinateMarqueeScroll"];
    self.marqueeScrollDelay = [coder decodeDoubleForKey:@"marqueeScrollDelay"];
    self.coordinateMarqueeScroll =
        [coder decodeBoolForKey:@"coordinateMarqueeScroll"];
...
- (void)configureWithDefaultHighlightColor {
  _highlightColor =
      [[UIColor alloc] initWithDynamicProvider:^UIColor *_Nonnull(
