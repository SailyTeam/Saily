...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p chain: %@>", self.class, self, _chain];
}
...
- (id)objectForKey:(NSString *)key {
  ...
  [_chain enumerateObjectsUsingBlock:^(UIBarAppearance *_Nonnull obj,
...
- (void)forwardInvocation:(NSInvocation *)anInvocation {
  if (strncmp(anInvocation.methodSignature.methodReturnType, @encode(void),
              1) == 0) {
  ...
  if (strncmp(anInvocation.methodSignature.methodReturnType, @encode(id), 1) ==
      0) {
    // id
    id rv = [self objectForKey:key];
    [anInvocation setReturnValue:&rv];
  } else if (strncmp(anInvocation.methodSignature.methodReturnType,
                     @encode(NSInteger), 1) == 0 ||
             strncmp(anInvocation.methodSignature.methodReturnType,
                     @encode(NSUInteger), 1) == 0) {
    // Integers
