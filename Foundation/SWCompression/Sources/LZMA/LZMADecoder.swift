...
struct LZMADecoder {
    ...
    mutating func resetStateAndDecoders() {
        ...
        probabilities = Array(repeating: LZMAConstants.probInitValue, count: 2 * 192 + 4 * 12)
        literalProbs = Array(repeating: Array(repeating: LZMAConstants.probInitValue, count: 0x300),
                             count: 1 << (lc + lp))
        ...
        alignDecoder = LZMABitTreeDecoder(numBits: LZMAConstants.numAlignBits)
        posDecoders = Array(repeating: LZMAConstants.probInitValue,
    ...
    mutating func decode() throws {
        ...
        rangeDecoder = try LZMARangeDecoder(byteReader)
    ...
    private func byte(at distance: Int) -> UInt8 {
        out[distance <= dictEnd ? dictEnd - distance : dictSize - distance + dictEnd]
    }
}
