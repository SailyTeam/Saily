...
static const void *LNToolbarHiddenBeforeTransition =
    &LNToolbarHiddenBeforeTransition;
static const void *LNPopupAdjustingInsets = &LNPopupAdjustingInsets;
static const void *LNPopupAdditionalSafeAreaInsets =
    &LNPopupAdditionalSafeAreaInsets;
static const void *LNUserAdditionalSafeAreaInsets =
    &LNUserAdditionalSafeAreaInsets;
static const void *LNPopupChildAdditiveSafeAreaInsets =
    &LNPopupChildAdditiveSafeAreaInsets;
...
static BOOL
__LNPopupIsClassBuggyForAdditionalSafeArea(UIViewController *controller);

#ifndef LNPopupControllerEnforceStrictClean
//_hideBarWithTransition:isExplicit:
static NSString *const hBWTiEBase64 =
    @"X2hpZGVCYXJXaXRoVHJhbnNpdGlvbjppc0V4cGxpY2l0Og==";
//_showBarWithTransition:isExplicit:
static NSString *const sBWTiEBase64 =
    @"X3Nob3dCYXJXaXRoVHJhbnNpdGlvbjppc0V4cGxpY2l0Og==";
//_setToolbarHidden:edge:duration:
static NSString *const sTHedBase64 =
    @"X3NldFRvb2xiYXJIaWRkZW46ZWRnZTpkdXJhdGlvbjo=";
//_viewControllerUnderlapsStatusBar
static NSString *const vCUSBBase64 =
    @"X3ZpZXdDb250cm9sbGVyVW5kZXJsYXBzU3RhdHVzQmFy";
//_hideShowNavigationBarDidStop:finished:context:
static NSString *const hSNBDSfcBase64 =
    @"X2hpZGVTaG93TmF2aWdhdGlvbkJhckRpZFN0b3A6ZmluaXNoZWQ6Y29udGV4dDo=";
//_viewSafeAreaInsetsFromScene
static NSString *const vSAIFSBase64 =
    @"X3ZpZXdTYWZlQXJlYUluc2V0c0Zyb21TY2VuZQ==";
//_updateLayoutForStatusBarAndInterfaceOrientation
static NSString *const uLFSBAIO =
    @"X3VwZGF0ZUxheW91dEZvclN0YXR1c0JhckFuZEludGVyZmFjZU9yaWVudGF0aW9u";
//_accessibilitySpeakThisViewController
static NSString *const aSTVC =
    @"X2FjY2Vzc2liaWxpdHlTcGVha1RoaXNWaWV3Q29udHJvbGxlcg==";
...
static NSString *const uiNVCA =
    @"VUlOYXZpZ2F0aW9uQ29udHJvbGxlckFjY2Vzc2liaWxpdHk=";
...
__attribute__((constructor)) static void __accessibilityBundleLoadHandler() {
  __accessibilityBundleLoadObserver = [[NSNotificationCenter defaultCenter]
      addObserverForName:NSBundleDidLoadNotification
                  object:nil
                   queue:nil
              usingBlock:^(NSNotification *_Nonnull note) {
                NSBundle *bundle = note.object;
                if ([bundle.bundleURL.lastPathComponent
                        isEqualToString:@"UIKit.axbundle"] == NO) {
                  return;
                }
                ...
                Method m1 = class_getInstanceMethod(
                    NSClassFromString(clsName), NSSelectorFromString(selName));
                __orig_uiVCA_aSTVC = (void *)method_getImplementation(m1);
                Method m2 = class_getInstanceMethod(
                    [UIViewController class], NSSelectorFromString(@"_aSTVC"));
                method_exchangeImplementations(m1, m2);

                clsName = _LNPopupDecodeBase64String(uiNVCA);
                m1 = class_getInstanceMethod(NSClassFromString(clsName),
                                             NSSelectorFromString(selName));
                __orig_uiNVCA_aSTVC = (void *)method_getImplementation(m1);
                m2 = class_getInstanceMethod([UINavigationController class],
                                             NSSelectorFromString(@"_aSTVC"));
                method_exchangeImplementations(m1, m2);
                ...
                m2 = class_getInstanceMethod([UITabBarController class],
                                             NSSelectorFromString(@"_aSTVC"));
                method_exchangeImplementations(m1, m2);

                [[NSNotificationCenter defaultCenter]
                    removeObserver:__accessibilityBundleLoadObserver];
...
+ (void)load {
  @autoreleasepool {
    ...
    dispatch_once(&onceToken, ^{
      __LNPopupBuggyAdditionalSafeAreaClasses =
          [NSSet setWithObjects:UINavigationController.class,
                                UITabBarController.class, nil];

      LNSwizzleMethod(self, @selector(isModalInPresentation),
                      @selector(_ln_isModalInPresentation));

      LNSwizzleMethod(self, @selector(setOverrideUserInterfaceStyle:),
      ...
      LNSwizzleMethod(self, @selector(additionalSafeAreaInsets),
                      @selector(_ln_additionalSafeAreaInsets));

      LNSwizzleMethod(self, @selector(setAdditionalSafeAreaInsets:),
                      @selector(_ln_setAdditionalSafeAreaInsets:));

      LNSwizzleMethod(self, @selector(setNeedsStatusBarAppearanceUpdate),
                      @selector(_ln_setNeedsStatusBarAppearanceUpdate));

      LNSwizzleMethod(self, @selector(childViewControllerForStatusBarStyle),
                      @selector(_ln_childViewControllerForStatusBarStyle));

      LNSwizzleMethod(self, @selector(childViewControllerForStatusBarHidden),
                      @selector(_ln_childViewControllerForStatusBarHidden));

      LNSwizzleMethod(
          self, @selector(viewWillTransitionToSize:withTransitionCoordinator:),
          @selector(_ln_viewWillTransitionToSize:withTransitionCoordinator:));

      LNSwizzleMethod(self,
                      @selector(willTransitionToTraitCollection:
                                      withTransitionCoordinator:),
                      @selector(_ln_willTransitionToTraitCollection:
                                          withTransitionCoordinator:));

      LNSwizzleMethod(self,
                      @selector(presentViewController:animated:completion:),
                      @selector(_ln_presentViewController:
                                                 animated:completion:));

#ifndef LNPopupControllerEnforceStrictClean
      //_viewControllerUnderlapsStatusBar
      NSString *selName = _LNPopupDecodeBase64String(vCUSBBase64);
      LNSwizzleMethod(self, NSSelectorFromString(selName), @selector(_vCUSB));

      //_updateLayoutForStatusBarAndInterfaceOrientation
      selName = _LNPopupDecodeBase64String(uLFSBAIO);
      LNSwizzleMethod(self, NSSelectorFromString(selName),
                      @selector(_uLFSBAIO));
      ...
      LNSwizzleMethod(self, NSSelectorFromString(selName), @selector(_vSAIFS));
#endif
    });
  }
}
...
- (BOOL)_ln_isModalInPresentation {
  if (self._ln_popupController_nocreate.popupControllerInternalState >=
      _LNPopupPresentationStateTransitioning) {
    return YES;
  }
...
- (void)_ln_popup_setOverrideUserInterfaceStyle:
  ...
  if (self._isContainedInPopupController) {
    [self.popupPresentationContainerViewController.popupContentView
        setControllerOverrideUserInterfaceStyle:overrideUserInterfaceStyle];
  }
}

static inline __attribute__((always_inline)) void
_LNUpdateUserSafeAreaInsets(id self, UIEdgeInsets userEdgeInsets,
                            UIEdgeInsets popupUserEdgeInsets) {
...
static inline __attribute__((always_inline)) void
_LNSetPopupSafeAreaInsets(id self, UIEdgeInsets additionalSafeAreaInsets) {
  objc_setAssociatedObject(
      self, LNPopupAdditionalSafeAreaInsets,
      [NSValue valueWithUIEdgeInsets:additionalSafeAreaInsets],
...
- (void)_ln_setAdditionalSafeAreaInsets:(UIEdgeInsets)additionalSafeAreaInsets {
  objc_setAssociatedObject(
      self, LNUserAdditionalSafeAreaInsets,
      [NSValue valueWithUIEdgeInsets:additionalSafeAreaInsets],
      OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)_ln_setChildAdditiveSafeAreaInsets:
    (UIEdgeInsets)childAdditiveSafeAreaInsets {
  objc_setAssociatedObject(
      self, LNPopupChildAdditiveSafeAreaInsets,
      [NSValue valueWithUIEdgeInsets:childAdditiveSafeAreaInsets],
      OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

UIEdgeInsets _LNPopupSafeAreas(id self) {
  return [objc_getAssociatedObject(self, LNPopupAdditionalSafeAreaInsets)
      UIEdgeInsetsValue];
}

static inline __attribute__((always_inline)) UIEdgeInsets
_LNUserSafeAreas(id self) {
  return [objc_getAssociatedObject(self, LNUserAdditionalSafeAreaInsets)
      UIEdgeInsetsValue];
}

UIEdgeInsets _LNPopupChildAdditiveSafeAreas(id self) {
  return [objc_getAssociatedObject(self, LNPopupChildAdditiveSafeAreaInsets)
      UIEdgeInsetsValue];
}
...
- (void)_ln_presentViewController:(UIViewController *)viewControllerToPresent
                         animated:(BOOL)flag
                       completion:(void (^)(void))completion {
  if (self.popupPresentationContainerViewController) {
    [self.popupPresentationContainerViewController
        presentViewController:viewControllerToPresent
                     animated:flag
                   completion:completion];
  } else {
    [self _ln_presentViewController:viewControllerToPresent
                           animated:flag
                         completion:completion];
  }
}
...
- (void)_ln_viewWillTransitionToSize:(CGSize)size
           withTransitionCoordinator:
               (id<UIViewControllerTransitionCoordinator>)coordinator {
  if (self._ln_popupController_nocreate) {
    [self.popupContentViewController viewWillTransitionToSize:size
                                    withTransitionCoordinator:coordinator];
  }

  if (self._ln_popupController_nocreate.popupBar.customBarViewController !=
      nil) {
    [self._ln_popupController_nocreate.popupBar
        _transitionCustomBarViewControllerWithPopupContainerSize:size
                                                 withCoordinator:coordinator];
  }

  [self _ln_viewWillTransitionToSize:size
           withTransitionCoordinator:coordinator];
}

- (void)_ln_willTransitionToTraitCollection:(UITraitCollection *)newCollection
                  withTransitionCoordinator:
                      (id<UIViewControllerTransitionCoordinator>)coordinator {
  if (self._ln_popupController_nocreate) {
    [self.popupContentViewController
  ...
  if (self._ln_popupController_nocreate.popupBar.customBarViewController !=
      nil) {
    [self._ln_popupController_nocreate.popupBar
        _transitionCustomBarViewControllerWithPopupContainerTraitCollection:
            newCollection
                                                            withCoordinator:
                                                                coordinator];
  }

  [self _ln_willTransitionToTraitCollection:newCollection
                  withTransitionCoordinator:coordinator];
}
...
- (UIViewController *)_findAncestorParentPopupContainerController {
  ...
  return
      [self.parentViewController _findAncestorParentPopupContainerController];
}
...
- (nullable UIViewController *)_common_childviewControllersForStatusBarLogic {
  ...
  CGFloat statusBarHeight = [LNPopupController
      _statusBarHeightForView:self.isViewLoaded ? self.view : nil];

  if ((vcToCheckForPopupPresentation._ln_popupController_nocreate
           .popupControllerTargetState == LNPopupPresentationStateOpen) ||
      (vcToCheckForPopupPresentation._ln_popupController_nocreate
               .popupControllerTargetState >
           LNPopupPresentationStateBarPresented &&
       vcToCheckForPopupPresentation._ln_popupController_nocreate
               .popupContentView.frame.origin.y <= (statusBarHeight / 2))) {
...
- (nullable UIViewController *)
    _ln_common_childViewControllerForStatusBarHidden {
...
- (UIViewController *)_aSTVC {
  if (self.popupContentViewController &&
      self.popupPresentationState == LNPopupPresentationStateOpen) {
...
- (void)_common_uLFSBAIO {
  ...
  if (self.popupContentViewController) {
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      //delay:0.0 usingSpringWithDamping:500 initialSpringVelocity:0.0 options:
      ...
      [self._ln_popupController_nocreate
              .popupContentView _repositionPopupCloseButton];
      //			} completion:nil];
    });
  }
#endif
...
- (UIEdgeInsets)_vSAIFS {
  if ([self _isContainedInPopupController]) {
    return __LNEdgeInsetsSum(
        self.popupPresentationContainerViewController.view.safeAreaInsets,
        UIEdgeInsetsMake(
            0, 0,
            -_LNPopupSafeAreas(self.popupPresentationContainerViewController)
                 .bottom,
            0));
  }
...
- (BOOL)_vCUSB {
  if ([self _isContainedInPopupController]) {
    UIViewController *statusBarVC =
        [self childViewControllerForStatusBarHidden] ?: self;
...
- (void)_layoutPopupBarOrderForTransition {
  [self._ln_popupController_nocreate.popupBar.superview
      insertSubview:self._ln_popupController_nocreate.popupBar
       aboveSubview:self.bottomDockingViewForPopup_internalOrDeveloper];
  [self._ln_popupController_nocreate.popupBar.superview
      insertSubview:self._ln_bottomBarExtension_nocreate
       belowSubview:self._ln_popupController_nocreate.popupBar];
  [self._ln_popupController_nocreate.popupBar.superview
      insertSubview:self._ln_popupController_nocreate.popupContentView
       belowSubview:self._ln_popupController_nocreate.popupBar];
}

- (void)_layoutPopupBarOrderForUse {
  [self.bottomDockingViewForPopup_internalOrDeveloper.superview
      bringSubviewToFront:self.bottomDockingViewForPopup_internalOrDeveloper];
  [self._ln_popupController_nocreate.popupBar.superview
      insertSubview:self._ln_popupController_nocreate.popupBar
       belowSubview:self.bottomDockingViewForPopup_internalOrDeveloper];
  [self._ln_popupController_nocreate.popupBar.superview
      insertSubview:self._ln_bottomBarExtension_nocreate
       belowSubview:self._ln_popupController_nocreate.popupBar];
  [self._ln_popupController_nocreate.popupBar.superview
...
- (_LNPopupBarBackgroundView *)_ln_bottomBarExtension {
  ...
  _LNPopupBarBackgroundView *rv =
      objc_getAssociatedObject(self, LNPopupBarExtensionView);
  if (rv == nil) {
    rv = [[_LNPopupBarBackgroundView alloc]
        initWithEffect:
            [UIBlurEffect
                effectWithStyle:UIBlurEffectStyleSystemChromeMaterial]];
    objc_setAssociatedObject(self, LNPopupBarExtensionView, rv,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)_ln_popup_viewDidLayoutSubviews {
  ...
  if (self.bottomDockingViewForPopup_nocreateOrDeveloper != nil) {
    if (self.bottomDockingViewForPopup_nocreateOrDeveloper ==
        self._ln_bottomBarSupport_nocreate) {
      self._ln_bottomBarSupport_nocreate.frame =
      ...
      self._ln_bottomBarExtension.frame =
          self._ln_bottomBarSupport_nocreate.frame;
    } else {
    ...
    if (self._ignoringLayoutDuringTransition == NO &&
        self._ln_popupController_nocreate.popupControllerInternalState !=
            LNPopupPresentationStateBarHidden) {
      [self._ln_popupController_nocreate
          _setContentToState:self._ln_popupController_nocreate
                                 .popupControllerInternalState];
    }
  ...
  if (self.shouldExtendPopupBarUnderSafeArea == NO) {
    removeFromSuperview();
  } else if (self._ln_popupController_nocreate.popupControllerInternalState ==
                 LNPopupPresentationStateBarHidden &&
             extensionView.superview != nil) {
...
- (void)_setIgnoringLayoutDuringTransition:
    (BOOL)ignoringLayoutDuringTransition {
  objc_setAssociatedObject(self, LNPopupAdjustingInsets,
                           @(ignoringLayoutDuringTransition),
...
static void __LNPopupUpdateChildInsets(UIViewController *controller) {
  ...
  while (parentViewController != nil &&
         __LNPopupIsClassBuggyForAdditionalSafeArea(parentViewController) ==
             YES) {
    popupSafeAreaInsets =
        __LNEdgeInsetsSum(popupSafeAreaInsets,
                          _LNPopupChildAdditiveSafeAreas(parentViewController));
...
void _LNPopupSupportSetPopupInsetsForViewController(
    UIViewController *controller, BOOL layout, UIEdgeInsets popupEdgeInsets) {
  // Container classes with bottom bars have bugs if additional safe areas are
  // applied directly to them. Instead, set a custom property and update their
  // children recursively to take care of the additional safe area.
  if (__LNPopupIsClassBuggyForAdditionalSafeArea(controller) == YES) {
...
- (BOOL)_isTabBarHiddenDuringTransition {
  NSNumber *isHidden =
      objc_getAssociatedObject(self, LNToolbarHiddenBeforeTransition);
  return isHidden.boolValue;
}

- (void)_setTabBarHiddenDuringTransition:(BOOL)toolbarHidden {
  objc_setAssociatedObject(self, LNToolbarHiddenBeforeTransition,
                           @(toolbarHidden), OBJC_ASSOCIATION_RETAIN);
}

- (BOOL)_isPrepareTabBarIgnored {
  NSNumber *isHidden =
      objc_getAssociatedObject(self, LNPopupIgnorePrepareTabBar);
  return isHidden.boolValue;
}

- (void)_setPrepareTabBarIgnored:(BOOL)isPrepareTabBarIgnored {
  objc_setAssociatedObject(self, LNPopupIgnorePrepareTabBar,
                           @(isPrepareTabBarIgnored), OBJC_ASSOCIATION_RETAIN);
}
...
- (UIEdgeInsets)insetsForBottomDockingView {
  return self.tabBar.hidden == NO && self._isTabBarHiddenDuringTransition == NO
             ? UIEdgeInsetsZero
             : self.view.superview.safeAreaInsets;
}
...
- (CGRect)defaultFrameForBottomDockingView {
  ...
  if (NSProcessInfo.processInfo.operatingSystemVersion.majorVersion < 13) {
    ...
    bottomBarFrame.size.height =
        MAX(bottomBarFrame.size.height, bottomBarSizeThatFits.height);
  }
#endif

  bottomBarFrame.origin = CGPointMake(
      0, self.view.bounds.size.height - (self._isTabBarHiddenDuringTransition
                                             ? 0.0
                                             : bottomBarFrame.size.height));
...
+ (void)load {
  ...
  dispatch_once(&onceToken, ^{
    ...
    LNSwizzleMethod(self, NSSelectorFromString(selName), @selector(hBWT:iE:));

    //_showBarWithTransition:isExplicit:
    selName = _LNPopupDecodeBase64String(sBWTiEBase64);
    LNSwizzleMethod(self, NSSelectorFromString(selName), @selector(sBWT:iE:));

    //_updateLayoutForStatusBarAndInterfaceOrientation
    selName = _LNPopupDecodeBase64String(uLFSBAIO);
    LNSwizzleMethod(self, NSSelectorFromString(selName), @selector(_uLFSBAIO));

    selName = _LNPopupDecodeBase64String(pTBBase64);
    LNSwizzleMethod(self, NSSelectorFromString(selName), @selector(_ln_pTB));
#endif
  });
}
...
- (void)_ln_popup_viewDidLayoutSubviews_tvc {
  if (self._ln_popupController_nocreate.popupControllerInternalState !=
    ...
    if (self.tabBar.isHidden == NO &&
        self._isTabBarHiddenDuringTransition == NO &&
        self._ignoringLayoutDuringTransition == NO) {
  ...
  struct objc_super superInfo = {self, [UIViewController class]};
  void (*super_call)(struct objc_super *, SEL) =
  ...
  if (self._ignoringLayoutDuringTransition == NO) {
    ...
    self._ln_bottomBarExtension_nocreate.frame =
        CGRectMake(0, self.view.bounds.size.height - bottomSafeArea,
                   self.view.bounds.size.width, bottomSafeArea);
  }
}
...
- (void)__repositionPopupBarToClosed_hack {
  ...
  frame.origin.y = defaultFrame.origin.y - frame.size.height -
                   self.insetsForBottomDockingView.bottom;
...
- (void)hBWT:(NSInteger)t iE:(BOOL)e {
  ...
  NSString *effectGroupingIdentifier =
      self._ln_popupController_nocreate.popupBar.effectGroupingIdentifier;
  ...
  if (t > 0) {
    ...
    [self.selectedViewController.transitionCoordinator
        animateAlongsideTransition:^(
          ...
          self._ln_bottomBarExtension_nocreate.frame = CGRectMake(
              0, self.view.bounds.size.height - bottomSafeArea,
              self.view.bounds.size.width,
              self._ln_bottomBarExtension_nocreate.frame.size.height);
          self._ln_popupController_nocreate.popupBar.bottomShadowView.alpha =
              0.0;
        ...
        completion:^(
          ...
          self._ln_popupController_nocreate.popupBar.bottomShadowView.hidden =
              YES;
          self._ln_popupController_nocreate.popupBar.bottomShadowView.alpha =
              1.0;

          self._ln_popupController_nocreate.popupBar.effectGroupingIdentifier =
              effectGroupingIdentifier;
...
- (CGRect)defaultFrameForBottomDockingView {
  ...
  if ([NSStringFromClass(self.nonMemoryLeakingPresentationController.class)
          containsString:@"Preview"] == NO) {
    bottomSafeAreaHeight -= self.view.window.safeAreaInsets.bottom;
  }

  toolbarBarFrame.origin = CGPointMake(
      toolbarBarFrame.origin.x,
...
- (UIEdgeInsets)insetsForBottomDockingView {
  if (self.presentingViewController != nil &&
      [NSStringFromClass(self.nonMemoryLeakingPresentationController.class)
          containsString:@"Preview"]) {
    return UIEdgeInsetsZero;
  }

  return UIEdgeInsetsMake(0, 0,
                          MAX(self.view.superview.safeAreaInsets.bottom,
                              self.view.window.safeAreaInsets.bottom),
                          0);
}
...
+ (void)load {
  ...
  dispatch_once(&onceToken, ^{
    ...
    LNSwizzleMethod(self, @selector(setNavigationBarHidden:animated:),
    ...
    LNSwizzleMethod(self, NSSelectorFromString(selName), @selector(_sTH:e:d:));
...
- (void)_sTH:(BOOL)hidden e:(UIRectEdge)edge d:(CGFloat)duration;
{
  ...
  void (^animations)(void) = ^{
    // During the transition, animate the popup bar and content together with
    // the toolbar transition.
    [self._ln_popupController_nocreate
        _setContentToState:self._ln_popupController_nocreate
                               .popupControllerInternalState];
    ...
    if (hidden) {
      self._ln_bottomBarExtension.frame =
          CGRectMake(0, self.view.bounds.size.height - safeArea,
                     self.view.bounds.size.width,
                     self._ln_bottomBarExtension_nocreate.frame.size.height);
  ...
  void (^completion)(BOOL finished) = ^(BOOL finished) {
    if (hidden) {
      self._ln_bottomBarExtension_nocreate.frame =
          CGRectMake(0, self.view.bounds.size.height - safeArea,
                     self.view.bounds.size.width, safeArea);
    }

    // Position the popup bar and content to the superview of the toolbar for
    // the transition.
    [self._ln_popupController_nocreate
      _setContentToState:self._ln_popupController_nocreate
                             .popupControllerInternalState];
...
@interface UISplitViewController (LNPopupSupportPrivate)
@end
...
+ (void)load {
  ...
  dispatch_once(&onceToken, ^{
    LNSwizzleMethod(self, @selector(viewDidLayoutSubviews),
                    @selector
                    (_ln_popup_viewDidLayoutSubviews_SplitViewNastyApple));
  });
}
...
- (void)_ln_popup_viewDidLayoutSubviews_SplitViewNastyApple {
  ...
  if (self.bottomDockingViewForPopup_nocreateOrDeveloper != nil) {
    // Apple forgot to call the super implementation of viewDidLayoutSubviews,
    // but we need that to layout the popup bar correctly.
    struct objc_super superInfo = {self, [UIViewController class]};
    void (*super_call)(struct objc_super *, SEL) =
        (void (*)(struct objc_super *, SEL))objc_msgSendSuper;
    super_call(&superInfo, @selector(viewDidLayoutSubviews));
...
- (void)viewWillAppear:(BOOL)animated {
  if (self._isContainedInPopupControllerOrDeallocated &&
      self._ln_isInPopupAppearanceTransition == NO) {
    return;
  }

  Class superclass = LNDynamicSubclassSuper(
      self, _LN_UIViewController_AppearanceControl.class);
  struct objc_super super = {.receiver = self, .super_class = superclass};
  void (*super_class)(struct objc_super *, SEL, BOOL) =
...
- (Class)class {
  return LNDynamicSubclassSuper(self,
