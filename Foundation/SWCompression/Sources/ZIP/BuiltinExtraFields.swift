...
struct ExtendedTimestampExtraField: ZipExtraField {
    ...
    init(_ byteReader: LittleEndianByteReader, _ size: Int, location: ZipExtraFieldLocation) {
        ...
        case .localHeader:
            ...
            if flags & 0x02 != 0 {
                atime = byteReader.uint32()
            }
            if flags & 0x04 != 0 {
                ctime = byteReader.uint32()
            }
        }
...
struct NtfsExtraField: ZipExtraField {
    ...
    init?(_ byteReader: LittleEndianByteReader, _ size: Int, location: ZipExtraFieldLocation) {
        ...
        mtime = byteReader.uint64()
        atime = byteReader.uint64()
        ctime = byteReader.uint64()
    }
}
...
struct InfoZipUnixExtraField: ZipExtraField {
    ...
    init?(_ byteReader: LittleEndianByteReader, _ size: Int, location: ZipExtraFieldLocation) {
        ...
        case .centralDirectory:
            return nil
        case .localHeader:
            uid = byteReader.int(fromBytes: 2)
            gid = byteReader.int(fromBytes: 2)
        }
    }
}
...
struct InfoZipNewUnixExtraField: ZipExtraField {
    ...
    init?(_ byteReader: LittleEndianByteReader, _ size: Int, location: ZipExtraFieldLocation) {
        ...
        if uidSize > 8 {
            ...
            uid = byteReader.int(fromBytes: uidSize)
        }
        ...
        if gidSize > 8 {
            ...
            gid = byteReader.int(fromBytes: gidSize)
        }
    }
}
