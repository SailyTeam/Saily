...
extern LNPopupInteractionStyle
...
@interface LNPopupInteractionPanGestureRecognizerDelegate
    : LNForwardingDelegate <UIGestureRecognizerDelegate>
@end
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
  LNPopupInteractionStyle resolvedStyle =
      _LNPopupResolveInteractionStyleFromInteractionStyle(
          _popupController.containerController.popupInteractionStyle);

  BOOL rv = resolvedStyle != LNPopupInteractionStyleNone;

  if (rv &&
      gestureRecognizer.view ==
          _popupController.currentContentController.view &&
      [_popupController.currentContentController
          respondsToSelector:@selector(_ln_interactionLimitRect)]) {
    CGRect limit =
        [_popupController.currentContentController _ln_interactionLimitRect];
  ...
  if (rv && [self.forwardedDelegate respondsToSelector:_cmd]) {
    return
        [self.forwardedDelegate gestureRecognizerShouldBegin:gestureRecognizer];
  }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer
    shouldBeRequiredToFailByGestureRecognizer:
        (UIGestureRecognizer *)otherGestureRecognizer {
  // This is to disable gesture recognizers in the superview while dragging the
  // popup bar. This is mostly to fix issues when the bar is part of a scroll
  // view scene, such as `UITableViewController` / `UITableView`.
  if ([_popupController.popupBar.superview.gestureRecognizers
          containsObject:otherGestureRecognizer]) {
    return YES;
  }
...
- (instancetype)initWithTarget:(id)target
                        action:(SEL)action
               popupController:(LNPopupController *)popupController {
  self = [super initWithTarget:target action:action];

  if (self) {
    _actualDelegate = [[LNPopupInteractionPanGestureRecognizerDelegate alloc]
        initWithPopupController:popupController];
...
- (void)setDelegate:(id<UIGestureRecognizerDelegate>)delegate {
  _actualDelegate = [[LNPopupInteractionPanGestureRecognizerDelegate alloc]
      initWithPopupController:[_actualDelegate valueForKey:@"popupController"]];
