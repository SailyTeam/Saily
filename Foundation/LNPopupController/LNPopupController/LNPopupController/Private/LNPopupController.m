...
LNPopupInteractionStyle _LNPopupResolveInteractionStyleFromInteractionStyle(
    LNPopupInteractionStyle style) {
...
OS_ALWAYS_INLINE
static void _LNCallDelegateObjectBool(UIViewController *controller,
  ...
  if ([controller.popupPresentationDelegate respondsToSelector:selector]) {
    ...
    msgSendObjectBool(controller.popupPresentationDelegate, selector,
                      controller, animated);
  }
}
...
- (instancetype)initWithContainerViewController:
    (__kindof UIViewController *)containerController {
  self = [super init];
...
- (CGRect)_frameForOpenPopupBar {
  //	CGRect defaultFrame = [_containerController
  //defaultFrameForBottomDockingView_internalOrDeveloper];
  return CGRectMake(0, -self.popupBar.frame.size.height,
                    _containerController.view.bounds.size.width,
                    self.popupBar.frame.size.height);
}
...
- (CGRect)_frameForClosedPopupBar {
  ...
  return CGRectMake(0,
                    defaultFrame.origin.y - self.popupBar.frame.size.height -
                        insets.bottom,
                    _containerController.view.bounds.size.width,
                    self.popupBar.frame.size.height);
}
...
- (void)_repositionPopupContentMovingBottomBar:(BOOL)bottomBar {
  ...
  CGFloat barHeight =
      (_bottomBar.isHidden ? 0 : _bottomBar.bounds.size.height) +
      _cachedInsets.bottom;
  CGFloat heightForContent = _containerController.view.bounds.size.height -
                             (1.0 - percent) * barHeight;

  if (bottomBar) {
    CGRect bottomBarFrame = _cachedDefaultFrame;
    bottomBarFrame.origin.y -= _cachedInsets.bottom;
    bottomBarFrame.origin.y +=
        (percent * (bottomBarFrame.size.height + _cachedInsets.bottom));
  ...
  contentFrame.origin.y =
      self.popupBar.frame.origin.y + self.popupBar.frame.size.height;

  CGFloat fractionalHeight =
      MAX(heightForContent -
              (self.popupBar.frame.origin.y + self.popupBar.frame.size.height),
          0);
  ...
  _containerController.popupContentViewController.view.frame =
      _containerController.view.bounds;

  [self.popupContentView _repositionPopupCloseButton];
}

static CGFloat __saturate(CGFloat x) { return MAX(0, MIN(1, x)); }
...
- (CGFloat)_percentFromPopupBar {
  return 1 - (CGRectGetMaxY(self.popupBar.frame) /
              (_cachedDefaultFrame.origin.y - _cachedInsets.bottom));
}
...
- (void)_setContentToState:(LNPopupPresentationState)state {
  ...
  if (state == LNPopupPresentationStateOpen) {
    targetFrame = [self _frameForOpenPopupBar];
             ...
             (state == _LNPopupPresentationStateTransitioning &&
              (_popupControllerTargetState ==
                   LNPopupPresentationStateBarHidden ||
               _popupControllerTargetState ==
  ...
  _cachedDefaultFrame = [_containerController
      defaultFrameForBottomDockingView_internalOrDeveloper];
  ...
  [self _repositionPopupContentMovingBottomBar:
            _containerController._ignoringLayoutDuringTransition == NO];
}

- (void)_addContentControllerSubview:
    (UIViewController *)currentContentController {
  if (currentContentController == nil) {
    return;
  }

  [self.popupContentView
    setControllerOverrideUserInterfaceStyle:currentContentController
  ...
  currentContentController.view.frame =
      self.popupContentView.contentView.bounds;
  [self.popupContentView.contentView addSubview:currentContentController.view];
}

- (void)_removeContentControllerFromContentView:
    (UIViewController *)currentContentController {
  if (currentContentController == nil) {
    return;
  }
...
- (void)_transitionToState:(LNPopupPresentationState)state
                    notifyDelegate:(BOOL)notifyDelegate
                          animated:(BOOL)animated
                useSpringAnimation:(BOOL)spring
    allowPopupBarAlphaModification:(BOOL)allowBarAlpha
                        completion:(void (^)(void))completion
        transitionOriginatedByUser:(BOOL)transitionOriginatedByUser {
  if (transitionOriginatedByUser == YES &&
      _popupControllerInternalState == _LNPopupPresentationStateTransitioning) {
    NSLog(@"LNPopupController: The popup controller is already in transition. "
          @"Will ignore this transition request.");
    return;
  }
  ...
  if (_popupControllerInternalState == LNPopupPresentationStateBarPresented) {
    ...
    [UIView performWithoutAnimation:^{
      //			_currentContentController.view.frame =
      //_containerController.view.bounds;

      [self.popupContentView
          _applyBackgroundEffectWithContentViewController:
              _currentContentController
                                                barEffect:(id)self.popupBar
                                                              .backgroundView
                                                              .effect];

      self.popupContentView.currentPopupContentViewController =
          _currentContentController;
      [self.popupContentView.contentView
          sendSubviewToBack:_currentContentController.view];
  ...
  if (_popupControllerPublicState == LNPopupPresentationStateOpen &&
    ...
    if (notifyDelegate) {
      _LNCallDelegateObjectBool(_containerController,
                                @selector
                                (popupPresentationControllerWillClosePopup:
                                                                  animated:),
                                animated);
    }
  }
  ...
  if (notifyDelegate &&
      ...
      state == LNPopupPresentationStateOpen) {
    _LNCallDelegateObjectBool(_containerController,
                              @selector
                              (popupPresentationControllerWillOpenPopup:
                                                               animated:),
                              animated);
  }
  ...
  LNPopupInteractionStyle resolvedStyle =
      _LNPopupResolveInteractionStyleFromInteractionStyle(
          _containerController.popupInteractionStyle);
  ...
  void (^updatePopupBarAlpha)(void) = ^{
    if (allowBarAlpha && resolvedStyle == LNPopupInteractionStyleSnap) {
      ...
      frame.size.height =
          state < _LNPopupPresentationStateTransitioning
              ? _LNPopupBarHeightForBarStyle(
                    _LNPopupResolveBarStyleFromBarStyle(self.popupBar.barStyle),
                    self.popupBar.customBarViewController)
              : 0.0;
  ...
  void (^completionBlock)(BOOL) = ^(BOOL finished) {
    ...
    if (state == LNPopupPresentationStateBarPresented) {
      ...
      [_currentContentController.viewForPopupInteractionGestureRecognizer
          removeGestureRecognizer:self.popupContentView
                                      .popupInteractionGestureRecognizer];
      [self.popupBar
          addGestureRecognizer:self.popupContentView
                                   .popupInteractionGestureRecognizer];
      ...
      if (_popupControllerPublicState == LNPopupPresentationStateOpen) {
        _LNCallDelegateObjectBool(_containerController,
                                  @selector
                                  (popupPresentationControllerDidClosePopup:
                                                                   animated:),
                                  animated);
      }
      ...
      UIAccessibilityPostNotification(UIAccessibilityScreenChangedNotification,
                                      _popupContentView.popupCloseButton);

      if (_popupControllerPublicState == LNPopupPresentationStateBarPresented) {
        _LNCallDelegateObjectBool(_containerController,
                                  @selector
                                  (popupPresentationControllerDidOpenPopup:
                                                                  animated:),
                                  animated);
      }
    }
  ...
  [UIView
       animateWithDuration:resolvedStyle == LNPopupInteractionStyleSnap ? 0.65
                                                                        : 0.5
                     delay:0.0
    usingSpringWithDamping:spring ? 0.8 : 1.0
...
- (void)_popupBarLongPressGestureRecognized:
    (UILongPressGestureRecognizer *)lpgr {
  if (self.popupBar.customBarViewController != nil &&
      self.popupBar.customBarViewController
              .wantsDefaultHighlightGestureRecognizer == NO) {
    return;
  }
...
- (void)_popupBarTapGestureRecognized:(UITapGestureRecognizer *)tgr {
  if (self.popupBar.customBarViewController != nil &&
      self.popupBar.customBarViewController.wantsDefaultTapGestureRecognizer ==
          NO) {
    return;
  }
  ...
  switch (tgr.state) {
  case UIGestureRecognizerStateEnded: {
    [self _transitionToState:_LNPopupPresentationStateTransitioning
                        notifyDelegate:NO
                              animated:NO
                    useSpringAnimation:NO
        allowPopupBarAlphaModification:NO
                            completion:^{
                              ...
                              [self _transitionToState:
                                        LNPopupPresentationStateOpen
                                                  notifyDelegate:YES
                                                        animated:YES
                                              useSpringAnimation:NO
                                  allowPopupBarAlphaModification:YES
                                                      completion:nil
...
- (void)_popupBarPresentationByUserPanGestureHandler_began:
  ...
  if (self.popupBar.customBarViewController != nil &&
      self.popupBar.customBarViewController.wantsDefaultPanGestureRecognizer ==
          NO) {
    return;
  }
  ...
  if (resolvedStyle == LNPopupInteractionStyleScroll &&
      [pgr.view isKindOfClass:UIScrollView.class] == NO) {
    // Ignore non-scroll events.
    return;
  }

  if (resolvedStyle == LNPopupInteractionStyleSnap) {
    if ((_popupControllerInternalState ==
             LNPopupPresentationStateBarPresented &&
         [pgr velocityInView:self.popupBar].y < 0)) {
      pgr.enabled = NO;
      pgr.enabled = YES;
      ...
      dispatch_after(
          ...
          dispatch_get_main_queue(), ^{
            [self _transitionToState:_popupControllerTargetState
                                notifyDelegate:YES
                                      animated:YES
                            useSpringAnimation:
                                _popupControllerTargetState ==
    ...
    } else if ((_popupControllerInternalState ==
                    LNPopupPresentationStateBarPresented &&
                [pgr velocityInView:self.popupBar].y > 0)) {
      pgr.enabled = NO;
      pgr.enabled = YES;
...
- (CGFloat)rubberbandFromHeight:(CGFloat)height {
   ...
   */
  CGFloat c = _containerController.popupSnapPercent, x = height,
          d = self.popupBar.superview.bounds.size.height;

  return (x * d * c) / (d + c * x);
}

- (void)_popupBarPresentationByUserPanGestureHandler_changed:
    (UIPanGestureRecognizer *)pgr {
#if TARGET_OS_MACCATALYST
  if (self.popupBar.window._ln_currentEvent.type ==
  ...
  if (pgr != _popupContentView.popupInteractionGestureRecognizer) {
    ...
    if ([possibleScrollView isKindOfClass:[UIScrollView class]]) {
      // If the scroll view has horizontal scroll, ignore the scroll view's pan
      // gesture recognizer.
      if (possibleScrollView.contentSize.width >
          possibleScrollView.bounds.size.width) {
        return;
      }

      id<UIGestureRecognizerDelegate> delegate =
          _popupContentView.popupInteractionGestureRecognizer.delegate;

      if (([delegate respondsToSelector:@selector
                     (gestureRecognizer:
                         shouldRequireFailureOfGestureRecognizer:)] &&
           [delegate gestureRecognizer:_popupContentView
                                           .popupInteractionGestureRecognizer
               shouldRequireFailureOfGestureRecognizer:pgr] == YES) ||
          ([delegate
           ...
           [delegate gestureRecognizer:_popupContentView
                                           .popupInteractionGestureRecognizer
               shouldRecognizeSimultaneouslyWithGestureRecognizer:pgr] == NO) ||
          (_dismissGestureStarted == NO &&
           possibleScrollView.contentOffset.y >
               -(possibleScrollView.contentInset.top +
                 LNPopupBarDeveloperPanGestureThreshold))) {
        return;
      }

      if (_dismissGestureStarted == NO) {
        _dismissScrollViewStartingContentOffset =
            possibleScrollView.contentOffset.y;
      }

      if (_popupBar.frame.origin.y > _cachedOpenPopupFrame.origin.y) {
        possibleScrollView.contentOffset =
            CGPointMake(possibleScrollView.contentOffset.x,
                        _dismissScrollViewStartingContentOffset);
      }
    } else {
      return;
  ...
  if (_dismissGestureStarted == NO &&
      (resolvedStyle == LNPopupInteractionStyleDrag ||
       resolvedStyle == LNPopupInteractionStyleScroll ||
    ...
    _statusBarThresholdDir =
        _popupControllerInternalState == LNPopupPresentationStateOpen ? 1 : -1;
    ...
    if (pgr != _popupContentView.popupInteractionGestureRecognizer) {
      _dismissStartingOffset =
          [pgr translationInView:self.popupBar.superview].y;
    } else {
  ...
  if (_dismissGestureStarted == YES) {
    CGFloat targetCenterY =
        MIN(_lastPopupBarLocation.y +
                [pgr translationInView:self.popupBar.superview].y,
            _cachedDefaultFrame.origin.y - self.popupBar.frame.size.height / 2 -
                _cachedInsets.bottom) -
        _dismissStartingOffset;
    targetCenterY = MAX(targetCenterY, _cachedOpenPopupFrame.origin.y +
                                           self.popupBar.frame.size.height / 2);
    ...
    if (resolvedStyle == LNPopupInteractionStyleSnap) {
      ...
      targetCenterY -=
          (self.popupBar.frame.size.height / 2) +
          [self rubberbandFromHeight:self.popupBar.frame.size.height / -2];
    }
    ...
    if (resolvedStyle == LNPopupInteractionStyleSnap &&
        realTargetCenterY / self.popupBar.superview.bounds.size.height >
            _containerController.popupSnapPercent) {
      _dismissGestureStarted = NO;
      ...
      [self _transitionToState:_popupControllerTargetState
          ...
          allowPopupBarAlphaModification:YES
                              completion:^{
                                [_popupContentView.popupCloseButton
                                        _setButtonContainerStationary];
                              }
              transitionOriginatedByUser:NO];
    }

    CGFloat statusBarHeightThreshold =
        [LNPopupController _statusBarHeightForView:_containerController.view] /
        2.0;

    if ((_statusBarThresholdDir == 1 && currentCenterY < targetCenterY &&
         _popupContentView.frame.origin.y >= statusBarHeightThreshold) ||
        (_statusBarThresholdDir == -1 && currentCenterY > targetCenterY &&
         _popupContentView.frame.origin.y < statusBarHeightThreshold)) {
...
- (void)_popupBarPresentationByUserPanGestureHandler_endedOrCancelled:
    (UIPanGestureRecognizer *)pgr {
  LNPopupInteractionStyle resolvedStyle =
      _LNPopupResolveInteractionStyleFromInteractionStyle(
          _containerController.popupInteractionStyle);
  ...
  if (_dismissGestureStarted == YES) {
    ...
    if (resolvedStyle == LNPopupInteractionStyleDrag ||
      ...
      BOOL hasPassedHeighThreshold =
          _stateBeforeDismissStarted == LNPopupPresentationStateBarPresented
              ? barTransitionPercent > LNPopupBarGestureHeightPercentThreshold
              : barTransitionPercent <
                    (1.0 - LNPopupBarGestureHeightPercentThreshold);
    ...
    [self _transitionToState:targetState
                        notifyDelegate:YES
                              animated:YES
                    useSpringAnimation:NO
        allowPopupBarAlphaModification:YES
...
- (void)_popupBarPresentationByUserPanGestureHandler:
    (UIPanGestureRecognizer *)pgr {
...
- (void)_reconfigure_image {
  ...
  if (_currentPopupItem.image != nil &&
...
- (void)_reconfigure_progress {
  [UIView performWithoutAnimation:^{
    [self.popupBarStorage.progressView setProgress:_currentPopupItem.progress
                                          animated:NO];
  }];
}
...
- (void)_reconfigure_accessibilityLavel {
  self.popupBarStorage.accessibilityCenterLabel =
      _currentPopupItem.accessibilityLabel;
}

- (void)_reconfigure_accessibilityHint {
  self.popupBarStorage.accessibilityCenterHint =
      _currentPopupItem.accessibilityHint;
}

- (void)_reconfigure_accessibilityImageLabel {
  self.popupBarStorage.accessibilityImageLabel =
      _currentPopupItem.accessibilityImageLabel;
}

- (void)_reconfigure_accessibilityProgressLabel {
  self.popupBarStorage.accessibilityProgressLabel =
      _currentPopupItem.accessibilityProgressLabel;
}

- (void)_reconfigure_accessibilityProgressValue {
  self.popupBarStorage.accessibilityProgressValue =
      _currentPopupItem.accessibilityProgressValue;
}

- (void)_reconfigureBarItems {
  [self.popupBarStorage _delayBarButtonLayout];
  [self.popupBarStorage
      setLeadingBarButtonItems:_currentPopupItem.leadingBarButtonItems];
  [self.popupBarStorage
      setTrailingBarButtonItems:_currentPopupItem.trailingBarButtonItems];
...
- (void)_reconfigure_swiftuiImageController {
  if (_currentPopupItem.swiftuiImageController != nil) {
    self.popupBarStorage.swiftuiImageController =
        _currentPopupItem.swiftuiImageController;
  }
...
- (void)_popupItem:(LNPopupItem *)popupItem
  ...
  if (self.popupBarStorage.customBarViewController) {
    ...
    NSString *reconfigureSelector =
        [NSString stringWithFormat:@"_reconfigure_%@", key];
...
- (void)_reconfigureContentWithOldContentController:
            (__kindof UIViewController *)oldContentController
  ...
  _LNPopupTransitionCoordinator *coordinator =
      [_LNPopupTransitionCoordinator new];
  [newContentController
      willTransitionToTraitCollection:_containerController.traitCollection
            withTransitionCoordinator:coordinator];
  [newContentController
       viewWillTransitionToSize:_containerController.view.bounds.size
      withTransitionCoordinator:coordinator];
  ...
  if (oldContentController != nil) {
    [self.popupContentView.contentView insertSubview:newContentController.view
                                        belowSubview:oldContentController.view];
  } else {
    [self _addContentControllerSubview:newContentController];
    [self.popupContentView.contentView
        sendSubviewToBack:newContentController.view];
  }
  //	}
  ...
  if (_popupControllerPublicState == LNPopupPresentationStateOpen) {
    [newContentController.viewForPopupInteractionGestureRecognizer
        addGestureRecognizer:self.popupContentView
                                 .popupInteractionGestureRecognizer];
  }
  ...
  if (self.popupBarStorage.customBarViewController != nil) {
    ...
    NSArray<NSString *> *keys = @[
      @"title", @"subtitle", @"image", @"progress", @"leadingBarButtonItems",
      @"trailingBarButtonItems", @"accessibilityLavel", @"accessibilityHint",
      @"accessibilityImageLabel", @"accessibilityProgressLabel",
      @"accessibilityProgressValue"
    ];
    [keys enumerateObjectsUsingBlock:^(NSString *__nonnull key, NSUInteger idx,
                                       BOOL *__nonnull stop) {
...
- (void)_configurePopupBarFromBottomBar {
  self.popupBar.effectGroupingIdentifier =
      _bottomBar._effectGroupingIdentifierIfAvailable;
  // Schedule one more effect identifier refresh, in case it's not yet ready at
  // this point.
  ...
  // visualProvider.toolbarIsSmall
  if ([_bottomBar isKindOfClass:UIToolbar.class] &&
      [[_bottomBar valueForKeyPath:_LNPopupDecodeBase64String(vPTIS)]
          boolValue] == YES) {
    appearanceToUse = [(UIToolbar *)_bottomBar compactAppearance];
  }

  if (appearanceToUse == nil &&
      [_bottomBar respondsToSelector:@selector(standardAppearance)]) {
...
- (void)_updateBarExtensionStyleFromPopupBar {
  _containerController._ln_bottomBarExtension_nocreate.backgroundColor =
      _containerController.popupBar.backgroundColor;
  _containerController._ln_bottomBarExtension_nocreate.colorView
      .backgroundColor =
      _containerController.popupBar.backgroundView.colorView.backgroundColor;
  _containerController._ln_bottomBarExtension_nocreate.imageView.image =
      _containerController.popupBar.backgroundView.imageView.image;
  _containerController._ln_bottomBarExtension_nocreate.imageView.contentMode =
      _containerController.popupBar.backgroundView.imageView.contentMode;
  _containerController._ln_bottomBarExtension_nocreate.effect =
      _containerController.popupBar.backgroundView.effect;
  [_containerController.popupBar
      _applyGroupingIdentifierToVisualEffectView:
          _containerController._ln_bottomBarExtension_nocreate];
...
- (void)_movePopupBarAndContentToBottomBarSuperview {
  ...
  if ([_bottomBar.superview isKindOfClass:[UIScrollView class]]) {
    NSLog(
        @"Attempted to present popup bar %@ on top of a UIScrollView subclass "
        @"%@. This is unsupported and may result in unexpected behavior.",
        self.popupBar, _bottomBar.superview);
  }
  ...
  if (_bottomBar.superview != nil) {
    ...
    [self.popupBar.superview insertSubview:self.popupContentView
                              belowSubview:self.popupBar];
  } else {
    [_containerController.view addSubview:self.popupBar];
    [_containerController.view bringSubviewToFront:self.popupBar];
    [_containerController.view insertSubview:self.popupContentView
                                belowSubview:self.popupBar];
  }
}
...
- (LNPopupBar *)popupBarStorage {
  ...
  _popupBar.popupOpenGestureRecognizer =
      [[LNPopupOpenTapGestureRecognizer alloc]
          initWithTarget:self
                  action:@selector(_popupBarTapGestureRecognized:)];
  [_popupBar addGestureRecognizer:_popupBar.popupOpenGestureRecognizer];

  _popupBar.barHighlightGestureRecognizer =
      [[LNPopupLongPressGestureRecognizer alloc]
          initWithTarget:self
                  action:@selector(_popupBarLongPressGestureRecognized:)];
...
- (LNPopupContentView *)popupContentView {
  ...
  self.popupContentView = [[LNPopupContentView alloc]
      initWithFrame:_containerController.view.bounds];
  _popupContentView.clipsToBounds = YES;
  [_popupContentView.popupCloseButton addTarget:self
                                         action:@selector(_closePopupContent)
  ...
  _popupContentView.popupInteractionGestureRecognizer =
      [[LNPopupInteractionPanGestureRecognizer alloc]
           initWithTarget:self
                   action:@selector
                   (_popupBarPresentationByUserPanGestureHandler:)
          popupController:self];

  return _popupContentView;
}

- (void)dealloc {
  // Cannot use self.popupBar in this method because it returns nil when the
  // popup state is LNPopupPresentationStateBarHidden.
  if (_popupBar) {
    [_popupBar removeFromSuperview];
  }
}

static void __LNPopupControllerDeeplyEnumerateSubviewsUsingBlock(
    UIView *view, void (^block)(UIView *view)) {
  block(view);
...
- (void)_fixupGestureRecognizersForController:(UIViewController *)vc {
  __LNPopupControllerDeeplyEnumerateSubviewsUsingBlock(
      vc.viewForPopupInteractionGestureRecognizer, ^(UIView *view) {
        [view.gestureRecognizers enumerateObjectsUsingBlock:^(
                                     __kindof UIGestureRecognizer *_Nonnull obj,
                                     NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)_cleanupGestureRecognizersForController:(UIViewController *)vc {
  [vc.viewForPopupInteractionGestureRecognizer.gestureRecognizers
      enumerateObjectsUsingBlock:^(__kindof UIGestureRecognizer *_Nonnull obj,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        if ([obj isKindOfClass:[UIPanGestureRecognizer class]] &&
            obj != _popupContentView.popupInteractionGestureRecognizer) {
...
- (void)presentPopupBarAnimated:(BOOL)animated
                      openPopup:(BOOL)open
                     completion:(void (^)(void))completionBlock {
  UIViewController *old = _currentContentController;
  [self _reconfigureContentWithOldContentController:old
                               newContentController:
                                   _containerController
  ...
  if (_popupControllerTargetState == LNPopupPresentationStateBarHidden) {
    ...
    _LNCallDelegateObjectBool(_containerController,
                              @selector
                              (popupPresentationControllerWillPresentPopupBar:
                                                                     animated:),
                              animated);
    ...
    _bottomBar =
        _containerController.bottomDockingViewForPopup_internalOrDeveloper;
    ...
    [self.popupBar addGestureRecognizer:self.popupContentView
                                            .popupInteractionGestureRecognizer];
    ...
    [UIView animateWithDuration:animated ? 0.5 : 0.0
        ...
        animations:^{
          ...
          barFrame.size.height = _LNPopupBarHeightForBarStyle(
              _LNPopupResolveBarStyleFromBarStyle(self.popupBar.barStyle),
              self.popupBar.customBarViewController);
          ...
          _LNPopupSupportSetPopupInsetsForViewController(
              _containerController, YES,
              UIEdgeInsetsMake(0, 0, barFrame.size.height, 0));

          if (open) {
            _LNCallDelegateObjectBool(
                _containerController,
                @selector(popupPresentationControllerWillOpenPopup:animated:),
                animated);
        ...
        completion:^(BOOL finished) {
          if (!open) {
            _popupControllerInternalState =
                LNPopupPresentationStateBarPresented;
            [_containerController _ln_setPopupPresentationState:
                                      LNPopupPresentationStateBarPresented];
          }

          _LNCallDelegateObjectBool(
              _containerController,
              @selector(popupPresentationControllerDidPresentPopupBar:
                                                             animated:),
              animated);
          ...
          if (open) {
            _LNCallDelegateObjectBool(
                _containerController,
                @selector(popupPresentationControllerDidOpenPopup:animated:),
                animated);
          }
...
- (void)openPopupAnimated:(BOOL)animated
               completion:(void (^)(void))completionBlock {
...
- (void)closePopupAnimated:(BOOL)animated
  ...
  if (_popupControllerTargetState == LNPopupPresentationStateOpen) {
    ...
    [self _transitionToState:LNPopupPresentationStateBarPresented
                        notifyDelegate:YES
                              animated:animated
                    useSpringAnimation:resolvedStyle ==
...
- (void)dismissPopupBarAnimated:(BOOL)animated
                     completion:(void (^)(void))completionBlock {
  ...
  if (_popupControllerInternalState != LNPopupPresentationStateBarHidden) {
    void (^dismissalAnimationCompletionBlock)(void) = ^{
      ...
      _LNCallDelegateObjectBool(
          _containerController,
          @selector(popupPresentationControllerWillDismissPopupBar:animated:),
          animated);
      ...
      [UIView animateWithDuration:animated ? 0.5 : 0.0
          ...
          animations:^{
            ...
            _LNPopupSupportSetPopupInsetsForViewController(
                _containerController, YES, UIEdgeInsetsZero);
          }
          completion:^(BOOL finished) {
            _popupControllerInternalState = LNPopupPresentationStateBarHidden;

            [self _removeContentControllerFromContentView:
                      _currentContentController];

            CGRect bottomBarFrame = [_containerController
                defaultFrameForBottomDockingView_internalOrDeveloper];
            bottomBarFrame.origin.y -= _cachedInsets.bottom;
            _bottomBar.frame = bottomBarFrame;
            ...
            _LNCallDelegateObjectBool(
                _containerController,
                @selector(popupPresentationControllerDidDismissPopupBar:
                                                               animated:),
                animated);
    ...
    if (_popupControllerTargetState != LNPopupPresentationStateBarPresented) {
      ...
      [self _transitionToState:LNPopupPresentationStateBarPresented
                      ...
                      useSpringAnimation:resolvedStyle ==
                                         LNPopupInteractionStyleSnap
          allowPopupBarAlphaModification:YES
                              completion:dismissalAnimationCompletionBlock
              transitionOriginatedByUser:NO];
    } else {
...
- (void)_applicationWillEnterForeground {
  [self.popupBar
      _setTitleViewMarqueesPaused:_popupControllerInternalState !=
                                  LNPopupPresentationStateBarPresented];
}
...
- (void)_popupBarMetricsDidChange:(LNPopupBar *)bar {
  ...
  _LNPopupSupportSetPopupInsetsForViewController(
      _containerController, YES,
      UIEdgeInsetsMake(0, 0, self.popupBar.frame.size.height, 0));
}

- (void)_popupBarStyleDidChange:(LNPopupBar *)bar {
  [self _updateBarExtensionStyleFromPopupBar];
  [_containerController.popupBar
      _applyGroupingIdentifierToVisualEffectView:self.popupContentView
                                                     .effectView];
}

- (void)_popupBar:(LNPopupBar *)bar
    updateCustomBarController:(LNPopupCustomBarViewController *)customController
