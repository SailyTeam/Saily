...
const void *_LNPopupPresentationContainerViewControllerKey =
    &_LNPopupPresentationContainerViewControllerKey;
const void *_LNPopupContentViewControllerKey =
    &_LNPopupContentViewControllerKey;
static const void *_LNPopupInteractionStyleKey = &_LNPopupInteractionStyleKey;
static const void *_LNPopupInteractionSnapPercentKey =
    &_LNPopupInteractionSnapPercentKey;
static const void *_LNPopupBottomBarSupportKey = &_LNPopupBottomBarSupportKey;
static const void *_LNPopupIsInPopupAppearanceTransitionKey =
    &_LNPopupIsInPopupAppearanceTransitionKey;
static const void *_LNPopupShouldExtendUnderSafeAreaKey =
    &_LNPopupShouldExtendUnderSafeAreaKey;
...
@dynamic ln_popupController, popupPresentationContainerViewController,
    popupContentViewController, bottomBarSupport;
...
static NSString *const ePCIEBase64 =
    @"X2V4aXN0aW5nUHJlc2VudGF0aW9uQ29udHJvbGxlckltbWVkaWF0ZTplZmZlY3RpdmU6";
#endif
...
- (UIPresentationController *)nonMemoryLeakingPresentationController {
  ...
  static id (*nonLeakingPresentationController)(id, SEL, BOOL, BOOL) =
      (id(*)(id, SEL, BOOL, BOOL))objc_msgSend;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    sel = _LNPopupDecodeBase64String(ePCIEBase64);
  });

  return nonLeakingPresentationController(self, NSSelectorFromString(sel), NO,
                                          NO);
#else
  NSString *selector = [NSString
      stringWithFormat:@"_%@",
                       NSStringFromSelector(@selector(presentationController))];
...
- (void)presentPopupBarWithContentViewController:(UIViewController *)controller
                                       openPopup:(BOOL)openPopup
                                        animated:(BOOL)animated
                                      completion:(nullable void (^)(void))
                                                     completionBlock;
{
  LNDynamicallySubclass(controller,
                        _LN_UIViewController_AppearanceControl.class);

  if (self.view.window == nil) {
    [self.view _ln_letMeKnowWhenViewInWindowHierarchy:^(
                   dispatch_block_t completionBlockInWindow) {
      [self presentPopupBarWithContentViewController:controller
                                           openPopup:openPopup
                                            animated:NO
                                          completion:^{
                                            if (completionBlock) {
  ...
  if (controller == nil) {
    [NSException raise:NSInternalInconsistencyException
                format:@"Content view controller cannot be nil."];
  }

  if (controller == self) {
    [NSException raise:NSInternalInconsistencyException
                format:@"Content view controller cannot be the same as the "
  ...
  [self._ln_popupController presentPopupBarAnimated:animated
                                          openPopup:openPopup
...
- (void)openPopupAnimated:(BOOL)animated
  ...
  [self._ln_popupController_nocreate openPopupAnimated:animated
                                            completion:completionBlock];
}
...
- (void)closePopupAnimated:(BOOL)animated
  ...
  [self._ln_popupController_nocreate closePopupAnimated:animated
                                             completion:completionBlock];
}
...
- (void)dismissPopupBarAnimated:(BOOL)animated
  ...
  if (self.view.window == nil) {
    [self.view _ln_letMeKnowWhenViewInWindowHierarchy:^(
                   dispatch_block_t completionBlockInWindow) {
      [self dismissPopupBarAnimated:NO
                         completion:^{
                           if (completionBlock) {
                             completionBlock();
                           }
  ...
  [self._ln_popupController_nocreate
    dismissPopupBarAnimated:animated
                 completion:^{
                   // Cleanup
                   self.popupContentViewController
                       .popupPresentationContainerViewController = nil;
                   self.popupContentViewController = nil;

                   // The LNPopupController is no longer released here.
                   // There should be one popup controller per presenting
...
- (void)setPopupPresentationDelegate:
    (id<LNPopupPresentationDelegate>)popupPresentationDelegate {
  self._ln_popupController.userPopupPresentationDelegate =
...
- (BOOL)_isContainedInPopupControllerOrDeallocated {
  if (objc_getAssociatedObject(
          self, _LNPopupPresentationContainerViewControllerKey) != nil) {
    return YES;
  }
...
- (UIViewController *)popupPresentationContainerViewController {
  return [(_LNWeakRef *)objc_getAssociatedObject(
      self, _LNPopupPresentationContainerViewControllerKey) object];
}

- (void)setPopupPresentationContainerViewController:
    (UIViewController *)popupPresentationContainerViewController {
  [self willChangeValueForKey:@"popupPresentationContainerViewController"];
  _LNWeakRef *weakRef =
      [_LNWeakRef refWithObject:popupPresentationContainerViewController];
  objc_setAssociatedObject(self, _LNPopupPresentationContainerViewControllerKey,
                           weakRef, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)setPopupContentViewController:
    (UIViewController *)popupContentViewController {
  [self willChangeValueForKey:@"popupContentViewController"];
  objc_setAssociatedObject(self, _LNPopupContentViewControllerKey,
...
- (LNPopupItem *)popupItem {
  ...
  if (rv == nil) {
    rv = [LNPopupItem new];
    objc_setAssociatedObject(self, _LNPopupItemKey, rv,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (LNPopupInteractionStyle)popupInteractionStyle {
  return (LNPopupInteractionStyle)[objc_getAssociatedObject(
      self, _LNPopupInteractionStyleKey) unsignedIntegerValue];
}

- (void)setPopupInteractionStyle:
    (LNPopupInteractionStyle)popupInteractionStyle {
  objc_setAssociatedObject(self, _LNPopupInteractionStyleKey,
...
- (double)popupSnapPercent {
  NSNumber *rv =
      objc_getAssociatedObject(self, _LNPopupInteractionSnapPercentKey);
...
- (void)setPopupSnapPercent:(double)popupDragPercent {
  objc_setAssociatedObject(self, _LNPopupInteractionSnapPercentKey,
                           @(_ln_clamp(popupDragPercent, 0.1, 0.9)),
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (BOOL)_ln_isInPopupAppearanceTransition {
  return
      [objc_getAssociatedObject(self, _LNPopupIsInPopupAppearanceTransitionKey)
          boolValue];
}

- (void)_ln_beginAppearanceTransition:(BOOL)isAppearing
                             animated:(BOOL)animated {
  objc_setAssociatedObject(self, _LNPopupIsInPopupAppearanceTransitionKey, @YES,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)_ln_endAppearanceTransition {
  ...
  objc_setAssociatedObject(self, _LNPopupIsInPopupAppearanceTransitionKey, nil,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (LNPopupController *)_ln_popupController {
  ...
  if (rv == nil) {
    ...
    objc_setAssociatedObject(self, _LNPopupControllerKey, rv,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
  }
...
- (_LNPopupBottomBarSupport *)_ln_bottomBarSupport {
  ...
  if (rv == nil) {
    rv = [[_LNPopupBottomBarSupport alloc]
        initWithFrame:CGRectMake(0, self.view.bounds.size.height,
                                 self.view.bounds.size.width, 0)];

    objc_setAssociatedObject(self, _LNPopupBottomBarSupportKey, rv,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (CGRect)defaultFrameForBottomDockingView_internal {
  CGFloat safeAreaAddition =
      self.view.safeAreaInsets.bottom - _LNPopupSafeAreas(self).bottom;
  ...
  return CGRectMake(0, self.view.bounds.size.height - safeAreaAddition,
                    self.view.bounds.size.width, safeAreaAddition);
}

- (CGRect)defaultFrameForBottomDockingView_internalOrDeveloper {
  return [self bottomDockingViewForPopupBar] != nil
             ? [self defaultFrameForBottomDockingView]
             : [self defaultFrameForBottomDockingView_internal];
}

- (BOOL)shouldExtendPopupBarUnderSafeArea {
  return [(objc_getAssociatedObject(self, _LNPopupShouldExtendUnderSafeAreaKey)
               ?: @1) boolValue];
}

- (void)setShouldExtendPopupBarUnderSafeArea:
    (BOOL)shouldExtendPopupBarUnderSafeArea {
  objc_setAssociatedObject(self, _LNPopupShouldExtendUnderSafeAreaKey,
                           @(shouldExtendPopupBarUnderSafeArea),
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
