...
//  Sail
//
//  Created by Lakr Aream on 2019/12/20.
...
extension UIView {
    ...
    func shineAnimation() {
        ...
        DispatchQueue.main.async {
            self.removeOldSubLayers()
            self.creatNewSubLayers()
        }
    }

    private func animate(circleShape: CAShapeLayer, circleTransform: CAKeyframeAnimation, circleMask: CAShapeLayer, circleMaskTransform: CAKeyframeAnimation, imageShape: CAShapeLayer, imageTransform: CAKeyframeAnimation, lineShapes: [CAShapeLayer], lineStrokeStart: CAKeyframeAnimation, lineStrokeEnd: CAKeyframeAnimation, lineOpacity: CAKeyframeAnimation) {
        CATransaction.begin()
    ...
    /// 移除旧layer
    private func removeOldSubLayers() {
    ...
    /// 创建动画layer
    private func creatNewSubLayers(with duration: TimeInterval = 1.0) {
        ...
        circleShape.fillColor = UIColor.randomAsPudding.cgColor
        ...
        for i in 0 ..< 5 {
            ...
            line.strokeColor = UIColor.randomAsPudding.cgColor
            line.lineWidth = 2
            line.miterLimit = 2
            line.path = {
