...
static const void *LNPopupAwaitingViewInWindowHierarchyKey =
    &LNPopupAwaitingViewInWindowHierarchyKey;
...
+ (void)load {
  ...
  dispatch_once(&onceToken, ^{
    ...
    LNSwizzleMethod(self, NSSelectorFromString(sel), @selector(_ln__dMFW:tW:));
#else
...
- (void)_ln__dMFW:(UIWindow *)fromWindow tW:(UIWindow *)toWindow {
  ...
  if ([self.nextResponder isKindOfClass:UIViewController.class] &&
      [self.nextResponder
          respondsToSelector:@selector(_ln_popup_viewDidMoveToWindow)]) {
...
LNAlwaysInline static void _LNNotify(UIView *self,
                                     NSMutableArray<LNInWindowBlock> *waiting) {
  if (waiting.count == 0) {
...
- (void)_ln_setNotifying:(BOOL)notifying {
  objc_setAssociatedObject(self, LNPopupNotifyingKey, @(notifying),
...
- (void)_ln_notify {
  NSMutableArray<LNInWindowBlock> *waiting =
      objc_getAssociatedObject(self, LNPopupAwaitingViewInWindowHierarchyKey);
...
- (void)_ln_letMeKnowWhenViewInWindowHierarchy:(LNInWindowBlock)block {
  ...
  if (waiting == nil) {
    ...
    objc_setAssociatedObject(self, LNPopupAwaitingViewInWindowHierarchyKey,
                             waiting, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
  }
...
- (UIEvent *)_ln_currentEvent {
  ...
  if ([NSStringFromClass([hostingWindow class]) hasSuffix:@"Proxy"]) {
    // On Big Sur, the hosting window is abstracted behind a proxy object, but
    // we need the actual NSWindow
...
LNAlwaysInline id _LNPopupReturnScrollEdgeAppearanceOrStandardAppearance(
    UIView *self, SEL standardAppearanceSelector,
    SEL scrollEdgeAppearanceSelector) {
  UIViewController *vc = self._ln_containerController;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
  if (vc != nil &&
      vc._ln_popupController_nocreate.currentContentController != nil) {
...
@interface UIToolbar (ScrollEdgeSupport)
@end
...
+ (void)load {
  @autoreleasepool {
    if (@available(iOS 15.0, *)) {
      LNSwizzleMethod(self, @selector(scrollEdgeAppearance),
                      @selector(_lnpopup_scrollEdgeAppearance));
      LNSwizzleMethod(self, @selector(compactScrollEdgeAppearance),
                      @selector(_lnpopup_compactScrollEdgeAppearance));
    }
  }
}
...
- (void)_ln_triggerScrollEdgeAppearanceRefreshIfNeeded {
  if (@available(iOS 15.0, *)) {
    ...
    self.compactScrollEdgeAppearance =
        self._lnpopup_compactScrollEdgeAppearance;
    [self setNeedsLayout];
    [self layoutIfNeeded];
  }
}
...
- (UIToolbarAppearance *)_lnpopup_scrollEdgeAppearance {
  return _LNPopupReturnScrollEdgeAppearanceOrStandardAppearance(
      self, @selector(standardAppearance),
      @selector(_lnpopup_scrollEdgeAppearance));
}
...
@interface UITabBar (ScrollEdgeSupport)
@end
