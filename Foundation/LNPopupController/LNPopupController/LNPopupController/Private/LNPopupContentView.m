...
LNPopupCloseButtonStyle _LNPopupResolveCloseButtonStyleFromCloseButtonStyle(
    LNPopupCloseButtonStyle style) {
...
- (nonnull instancetype)initWithFrame:(CGRect)frame {
  ...
  if (self) {
    ...
    _popupCloseButton =
        [[LNPopupCloseButton alloc] initWithContainingContentView:self];
...
- (void)setCurrentPopupContentViewController:
    (UIViewController *)currentPopupContentViewController {
...
- (void)setPopupCloseButtonStyle:
    (LNPopupCloseButtonStyle)popupCloseButtonStyle {
  _popupCloseButtonStyle = popupCloseButtonStyle;

  LNPopupCloseButtonStyle buttonStyle =
      _LNPopupResolveCloseButtonStyleFromCloseButtonStyle(
          self.popupCloseButtonStyle);
  ...
  [UIView performWithoutAnimation:^{
    ...
    if ([_currentPopupContentViewController
            positionPopupCloseButton:self.popupCloseButton] == YES) {
      return;
    } else {
    ...
    if (buttonStyle != LNPopupCloseButtonStyleNone) {
      ...
      if (_popupCloseButtonTopConstraint == nil) {
        _popupCloseButtonTopConstraint = [self.popupCloseButton.topAnchor
            constraintEqualToAnchor:self.contentView.topAnchor
                           constant:buttonStyle == LNPopupCloseButtonStyleRound
                                        ? 12
                                        : 8];

        [NSLayoutConstraint
            activateConstraints:@[ _popupCloseButtonTopConstraint ]];
      }

      if (_popupCloseButtonLeadingConstraint == nil) {
        _popupCloseButtonLeadingConstraint =
            [self.popupCloseButton.leadingAnchor
                constraintEqualToAnchor:self.contentView.leadingAnchor
                               constant:12];
      }

      if (_popupCloseButtonCenterConstraint == nil) {
        _popupCloseButtonCenterConstraint = [self.popupCloseButton.centerXAnchor
...
- (void)_repositionPopupCloseButton {
  ...
  _popupCloseButtonTopConstraint.constant =
      self.popupCloseButton.style == LNPopupCloseButtonStyleRound ? 12 : 4;

  CGFloat windowTopSafeAreaInset = 0;

  if ([NSStringFromClass(_currentPopupContentViewController
                             .popupPresentationContainerViewController
                             .nonMemoryLeakingPresentationController.class)
          containsString:@"Fullscreen"]) {
    windowTopSafeAreaInset += self.window.safeAreaInsets.top;
  } else {
    UIView *viewToUse =
        _currentPopupContentViewController
            .popupPresentationContainerViewController.presentingViewController
  ...
  id hitTest = [_currentPopupContentViewController.view
        hitTest:CGPointMake(12, _popupCloseButtonTopConstraint.constant)
      withEvent:nil];
  UINavigationBar *possibleBar = (id)[self _view:hitTest
                      selfOrSuperviewKindOfClass:[UINavigationBar class]];
  if (possibleBar) {
    if (self.popupCloseButtonAutomaticallyUnobstructsTopBars)
      _popupCloseButtonTopConstraint.constant +=
          CGRectGetHeight(possibleBar.bounds);
    else
...
- (UIUserInterfaceStyle)overrideUserInterfaceStyle {
  return _userOverrideUserInterfaceStyle != UIUserInterfaceStyleUnspecified
             ? _userOverrideUserInterfaceStyle
             : _controllerOverrideUserInterfaceStyle;
}
...
- (void)setControllerOverrideUserInterfaceStyle:
    (UIUserInterfaceStyle)overrideUserInterfaceStyle {
...
- (void)_applyBackgroundEffectWithContentViewController:(UIViewController *)vc
                                              barEffect:
                                                  (UIBlurEffect *)barEffect {
...
- (void)setBackgroundStyle:(UIBlurEffectStyle)backgroundStyle {
  _backgroundEffect = backgroundStyle == LNBackgroundStyleInherit
                          ? nil
                          : [UIBlurEffect effectWithStyle:backgroundStyle];
}

- (UIBlurEffectStyle)backgroundStyle {
  return _backgroundEffect == nil
             ? LNBackgroundStyleInherit
             : [[_backgroundEffect valueForKey:@"style"] unsignedIntegerValue];
}
...
- (BOOL)animateAlongsideTransition:
            (void (^__nullable)(id<UIViewControllerTransitionCoordinatorContext>
                                    context))animation
                        completion:
                            (void (^__nullable)(
...
- (BOOL)
    animateAlongsideTransitionInView:(nullable UIView *)view
                           animation:
                               (void (^__nullable)(
                                   id<UIViewControllerTransitionCoordinatorContext>
                                       context))animation
...
- (void)notifyWhenInteractionChangesUsingBlock:
    (nonnull void (^)(
        id<UIViewControllerTransitionCoordinatorContext> _Nonnull))handler {
}
