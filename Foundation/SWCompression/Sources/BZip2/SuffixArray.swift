...
enum SuffixArray {
    ...
    private struct Summary {
        ...
        init(_ guessedSuffixArray: [Int], _ state: State) {
            ...
            suffixOffsets = summarySuffixOffsets
            alphabetSize = currentName + 1
            bytes = summaryBytes
        }

        func makeSuffixArray() -> [Int] {
            if alphabetSize == bytes.count {
                var summarySuffixArray = Array(repeating: -1, count: bytes.count + 1)
                summarySuffixArray[0] = bytes.count
                ...
                return SuffixArray.make(from: bytes, with: alphabetSize)
            }
        }
    }
