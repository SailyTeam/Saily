...
class TarCommand: Command {
    ...
    func execute() throws {
        var fileData: Data
        if create.value == nil {
            fileData = try Data(contentsOf: URL(fileURLWithPath: archive.value),
        ...
        if info.value {
        ...
        } else if let outputPath = extract.value {
        ...
        } else if let inputPath = create.value {
            let fileManager = FileManager.default

            guard !fileManager.fileExists(atPath: archive.value) else {
            ...
            if verbose.value {
                print("Creating new container at \"\(archive.value)\" from \"\(inputPath)\"")
                print("d = directory, f = file, l = symbolic link")
            }
            let entries = try createEntries(inputPath, verbose.value)
            let containerData = TarContainer.create(from: entries)
            try containerData.write(to: URL(fileURLWithPath: archive.value))
        }
    }
