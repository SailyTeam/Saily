...
NSString *const kMarqueeLabelControllerRestartNotification =
    @"MarqueeLabelViewControllerRestart";
NSString *const kMarqueeLabelShouldLabelizeNotification =
    @"MarqueeLabelShouldLabelizeNotification";
NSString *const kMarqueeLabelShouldAnimateNotification =
    @"MarqueeLabelShouldAnimateNotification";
NSString *const kMarqueeLabelAnimationCompletionBlock =
    @"MarqueeLabelAnimationCompletionBlock";

// Animation completion block
typedef void (^MLAnimationCompletionBlock)(BOOL finished);

// iOS Version check for iOS 8.0.0
#define SYSTEM_VERSION_IS_8_0_X                                                \
  ([[[UIDevice currentDevice] systemVersion] hasPrefix:@"8.0"])
...
- (CGFloat)durationPercentageForPositionPercentage:(CGFloat)positionPercentage
...
+ (void)notifyController:(UIViewController *)controller
...
- (void)viewControllerShouldRestart:(NSNotification *)notification {
  UIViewController *controller =
      [[notification userInfo] objectForKey:@"controller"];
...
- (void)forwardPropertiesToSubLabel {
  /*
   ...
   text properties are set via a Storyboard. As the Storyboard/IB doesn't
   currently support attributed strings, there's no need to "forward" the super
   attributedString value.
   */

  // Since we're a UILabel, we actually do implement all of UILabel's
  // properties. We don't care about these values, we just want to forward them
  // on to our sublabel.
  NSArray *properties = @[
    @"baselineAdjustment", @"enabled", @"highlighted", @"highlightedTextColor",
    @"minimumFontSize", @"textAlignment", @"userInteractionEnabled",
    @"adjustsFontSizeToFitWidth", @"lineBreakMode", @"numberOfLines",
    @"contentMode"
  ];
  ...
  self.subLabel.backgroundColor =
      (super.backgroundColor == nil ? [UIColor clearColor]
...
- (void)setupLabel {
  ...
  [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(viewControllerShouldRestart:)
             name:kMarqueeLabelControllerRestartNotification
           object:nil];
  [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(labelsShouldLabelize:)
             name:kMarqueeLabelShouldLabelizeNotification
           object:nil];
  [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(labelsShouldAnimate:)
             name:kMarqueeLabelShouldAnimateNotification
           object:nil];
}

- (void)minimizeLabelFrameWithMaximumSize:(CGSize)maxSize
  ...
  if (self.subLabel.text != nil) {
    ...
    CGSize minimumSize =
        CGSizeMake(minimumLabelSize.width + (self.fadeLength * 2),
                   minimumLabelSize.height);

    // Find minimum size of options
    minimumSize = CGSizeMake(MIN(minimumSize.width, maxSize.width),
                             MIN(minimumSize.height, maxSize.height));

    // Apply to frame
    self.frame = CGRectMake(
        self.frame.origin.x, self.frame.origin.y, minimumSize.width,
        (adjustHeight ? minimumSize.height : self.frame.size.height));
  }
}
...
- (void)updateSublabelAndBeginScroll:(BOOL)beginScroll {
  ...
  if (!self.labelShouldScroll) {
    ...
    case MLRightLeft:
    case MLRight:
      CGRectDivide(self.bounds, &unusedFrame, &labelFrame, self.leadingBuffer,
                   CGRectMaxXEdge);
      labelFrame = CGRectIntegral(labelFrame);
      break;

    default:
      labelFrame = CGRectIntegral(CGRectMake(
          self.leadingBuffer, 0.0f, self.bounds.size.width - self.leadingBuffer,
  ...
  CGFloat minTrailing =
      MAX(MAX(self.leadingBuffer, self.trailingBuffer), self.fadeLength);

  switch (self.marqueeType) {
  case MLContinuous:
  case MLContinuousReverse: {
    if (self.marqueeType == MLContinuous) {
      self.homeLabelFrame = CGRectIntegral(CGRectMake(self.leadingBuffer, 0.0f,
                                                      expectedLabelSize.width,
                                                      self.bounds.size.height));
      self.awayOffset = -(self.homeLabelFrame.size.width + minTrailing);
    } else {
      self.homeLabelFrame = CGRectIntegral(
          CGRectMake(self.bounds.size.width -
                         (expectedLabelSize.width + self.leadingBuffer),
                     0.0f, expectedLabelSize.width, self.bounds.size.height));
      self.awayOffset = (self.homeLabelFrame.size.width + minTrailing);
    }
    ...
    self.repliLayer.instanceTransform =
        CATransform3DMakeTranslation(-self.awayOffset, 0.0, 0.0);

    // Recompute the animation duration
    self.animationDuration =
        (self.rate != 0) ? ((NSTimeInterval)fabs(self.awayOffset) / self.rate)
                         : (self.scrollDuration);
  ...
  case MLRightLeft:
  case MLRight: {
    self.homeLabelFrame = CGRectIntegral(CGRectMake(
        self.bounds.size.width - (expectedLabelSize.width + self.leadingBuffer),
        0.0f, expectedLabelSize.width, self.bounds.size.height));
    self.awayOffset =
        (expectedLabelSize.width + self.trailingBuffer + self.leadingBuffer) -
  ...
  case MLLeft: {
    ...
    self.awayOffset =
        self.bounds.size.width -
        (expectedLabelSize.width + self.leadingBuffer + self.trailingBuffer);
...
#ifdef TARGET_OS_IOS
  // Sanitize width to 5461.0f (largest width a UILabel will draw on an iPhone
  // 6S Plus)
...
- (BOOL)labelShouldScroll {
  ...
  BOOL labelTooLarge = ([self subLabelSize].width + self.leadingBuffer >
                        self.bounds.size.width + FLT_EPSILON);
...
- (void)beginScrollWithDelay:(BOOL)delay {
  ...
  case MLContinuous:
  case MLContinuousReverse:
    [self scrollContinuousWithInterval:self.animationDuration
                                 after:(delay ? self.animationDelay : 0.0)];
    break;
  case MLLeft:
  case MLRight:
    [self scrollAwayWithInterval:self.animationDuration
                     delayAmount:(delay ? self.animationDelay : 0.0)shouldReturn
                                :NO];
    break;
  default:
    [self scrollAwayWithInterval:self.animationDuration];
...
- (void)scrollAwayWithInterval:(NSTimeInterval)interval
                   delayAmount:(NSTimeInterval)delayAmount
                  shouldReturn:(BOOL)shouldReturn {
  ...
  // Set Duration
  [CATransaction
      setAnimationDuration:(!shouldReturn ? CGFLOAT_MAX
                                          : 2.0 * (delayAmount + interval))];

  // Create animation for gradient, if needed
  if (self.fadeLength != 0.0f) {
    CAKeyframeAnimation *gradAnim =
        [self keyFrameAnimationForGradientFadeLength:self.fadeLength
                                            interval:interval
                                               delay:delayAmount];
  ...
  self.scrollCompletionBlock = ^(BOOL finished) {
    ...
    // 2) The instance is still attached to a window - this completion block is
    // called for
    //    many reasons, including if the animation is removed due to the view
    //    being removed from the UIWindow (typically when the view controller is
    //    no longer the "top" view)
    if (self.window && ![weakSelf.subLabel.layer animationForKey:@"position"]) {
      // Begin again, if conditions met
      if (weakSelf.labelShouldScroll && !weakSelf.tapToScroll &&
          !weakSelf.holdScrolling) {
        [weakSelf scrollAwayWithInterval:interval
  ...
  CGPoint awayOrigin =
      MLOffsetCGPoint(self.homeLabelFrame.origin, self.awayOffset);
  ...
  case MLRight:
    values = @[
      [NSValue valueWithCGPoint:homeOrigin], // Initial location, home
  ...
  CAKeyframeAnimation *awayAnim =
      [self keyFrameAnimationForProperty:@"position"
                                  values:values
                                interval:interval
                                   delay:delayAmount];
...
- (void)scrollContinuousWithInterval:(NSTimeInterval)interval
  ...
  self.scrollCompletionBlock = ^(BOOL finished) {
    ...
    if (weakSelf.window &&
        ![weakSelf.subLabel.layer animationForKey:@"position"]) {
      // Begin again, if conditions met
      if (weakSelf.labelShouldScroll && !weakSelf.tapToScroll &&
          !weakSelf.holdScrolling) {
...
- (void)applyGradientMaskForFadeLength:(CGFloat)fadeLength
  ...
  if (!CGRectEqualToRect(gradientMask.bounds, self.bounds)) {
    ...
    gradientMask.locations =
        @[ @(0.0f), @(leftFadeStop), @(1.0f - rightFadeStop), @(1.0f) ];
  }

  gradientMask.bounds = self.layer.bounds;
  gradientMask.position =
  ...
  case MLRight:
    adjustedColors = @[
      (trailingFadeNeeded ? transparent : opaque), opaque, opaque, opaque
    ];
    break;
  ...
  default:
    ...
    // MLLeftRight
    adjustedColors = @[
      opaque, opaque, opaque, (trailingFadeNeeded ? transparent : opaque)
    ];
    break;
  }
  ...
  if (animated) {
    ...
    GradientSetupAnimation *colorAnimation =
        [GradientSetupAnimation animationWithKeyPath:@"colors"];
...
- (CAKeyframeAnimation *)
    keyFrameAnimationForGradientFadeLength:(CGFloat)fadeLength
  ...
  CAKeyframeAnimation *animation =
      [CAKeyframeAnimation animationWithKeyPath:@"colors"];

  // Get timing function
  CAMediaTimingFunction *timingFunction =
      [self timingFunctionForAnimationOptions:self.animationCurve];
  ...
  case MLRightLeft:
    ...
    keyTimes = @[
      ...
      @(delayAmount /
        totalDuration), // 2) Begin of LE fade-in, just as scroll away starts
      @((delayAmount + 0.4) /
        totalDuration), // 3) End of LE fade in [LE fully faded]
      @((delayAmount + interval - 0.4) /
        totalDuration), // 4) Begin of TE fade out, just before scroll away
                        // finishes
      @((delayAmount + interval) /
        totalDuration), // 5) End of TE fade out [TE fade removed]
      @((delayAmount + interval + delayAmount) /
        totalDuration), // 6) Begin of TE fade back in, just as scroll home
                        // starts
      @((delayAmount + interval + delayAmount + 0.4) /
        totalDuration), // 7) End of TE fade back in [TE fully faded]
      @((totalDuration - 0.4) / totalDuration), // 8) Begin of LE fade out, just
                                                // before scroll home finishes
      @(1.0)
    ]; // 9) End of LE fade out, just as scroll home finishes
    break;
  ...
  default:
    ...
    CGFloat startFadeFraction =
        fabs((self.subLabel.bounds.size.width + self.leadingBuffer) /
             self.awayOffset);
    // Find when the animation will hit that point
    CGFloat startFadeTimeFraction = [timingFunction
        durationPercentageForPositionPercentage:startFadeFraction
                                   withDuration:totalDuration];
    NSTimeInterval startFadeTime =
        delayAmount + startFadeTimeFraction * interval;

    keyTimes = @[
      @(0.0),                         // Initial gradient
      @(delayAmount / totalDuration), // Begin of fade in
      @((delayAmount + 0.2) /
        totalDuration), // End of fade in, just as scroll away starts
      @((startFadeTime) /
        totalDuration), // Begin of fade out, just before scroll home completes
      @((startFadeTime + 0.1) /
        totalDuration), // End of fade out, as scroll home completes
  ...
  case MLContinuousReverse:
    values = @[
      ...
      ], // End of fade in, just as scroll away starts
      @[
        transp, opaque, opaque, transp
      ], // Begin of fade out, just before scroll home completes
      @[
        transp, opaque, opaque, opaque
      ], // End of fade out, as scroll home completes
...
- (CAKeyframeAnimation *)keyFrameAnimationForProperty:(NSString *)property
  ...
  // Create new animation
  CAKeyframeAnimation *animation =
      [CAKeyframeAnimation animationWithKeyPath:property];
  ...
  case MLRightLeft:
    NSAssert(
        values.count == 5,
        @"Incorrect number of values passed for MLLeftRight-type animation");
    totalDuration = 2.0 * (delayAmount + interval);
    // Set up keyTimes
    animation.keyTimes = @[
  ...
  case MLLeft:
  case MLRight:
    NSAssert(values.count == 4,
             @"Incorrect number of values passed for MLLeft-type animation");
    totalDuration = CGFLOAT_MAX;
    // Set up keyTimes
    animation.keyTimes = @[
      @(0.0),                                      // Initial location, home
  ...
  default:
    NSAssert(
        values.count == 3,
        @"Incorrect number of values passed for MLContinous-type animation");
    totalDuration = delayAmount + interval;
    // Set up keyTimes
    animation.keyTimes = @[
      @(0.0),                         // Initial location, home
...
- (CAMediaTimingFunction *)timingFunctionForAnimationOptions:
...
- (void)pauseLabel {
  // Only pause if label is not already paused, and already in a scrolling
  // animation
  if (!self.isPaused && self.awayFromHome) {
    // Pause sublabel position animation
    CFTimeInterval labelPauseTime =
    ...
    CFTimeInterval gradientPauseTime =
        [self.layer.mask convertTime:CACurrentMediaTime() fromLayer:nil];
...
- (void)setAnimationCurve:(UIViewAnimationOptions)animationCurve {
  ...
  NSUInteger allowableOptions = UIViewAnimationOptionCurveEaseIn |
...
- (void)setTapToScroll:(BOOL)tapToScroll {
  ...
  if (_tapToScroll) {
    UITapGestureRecognizer *newTapRecognizer = [[UITapGestureRecognizer alloc]
        initWithTarget:self
                action:@selector(labelWasTapped:)];
...
- (void)setHoldScrolling:(BOOL)holdScrolling {
  ...
  if (!holdScrolling &&
      !(self.awayFromHome || self.labelize || self.tapToScroll) &&
      self.labelShouldScroll) {
    [self beginScroll];
  }
}
...
- (NSArray *)gradientColors {
  if (!_gradientColors) {
    NSObject *transparent = (NSObject *)[[UIColor clearColor] CGColor];
    NSObject *opaque = (NSObject *)[[UIColor blackColor] CGColor];
    _gradientColors = [NSArray
        arrayWithObjects:transparent, opaque, opaque, transparent, nil];
  }
...
- (CGFloat)durationPercentageForPositionPercentage:(CGFloat)positionPercentage
                                      withDuration:(NSTimeInterval)duration {
  // Finds the animation duration percentage that corresponds with the given
  // animation "position" percentage. Utilizes Newton's Method to solve for the
  // parametric Bezier curve that is used by CAMediaAnimation.
  ...
  CGFloat durationPercentage = [self XforCurveAt:t_found
                               withControlPoints:controlPoints];

  return durationPercentage;
}

- (CGFloat)solveTForY:(CGFloat)y_0
          withEpsilon:(CGFloat)epsilon
  ...
  for (int i = 0; i < 15; i++) {
    ...
    // Check if derivative is small or zero (
    // http://en.wikipedia.org/wiki/Newton's_method#Failure_analysis )
    if (fabs(df0) < 1e-6) {
      break;
    }
    // Else recalculate t1
...
- (CGFloat)YforCurveAt:(CGFloat)t withControlPoints:(NSArray *)controlPoints {
  ...
  return powf((1 - t), 3) * P0.y + 3.0f * powf(1 - t, 2) * t * P1.y +
         3.0f * (1 - t) * powf(t, 2) * P2.y + powf(t, 3) * P3.y;
}
...
- (CGFloat)XforCurveAt:(CGFloat)t withControlPoints:(NSArray *)controlPoints {
  ...
  // Per http://en.wikipedia.org/wiki/Bezier_curve#Cubic_B.C3.A9zier_curves
  return powf((1 - t), 3) * P0.x + 3.0f * powf(1 - t, 2) * t * P1.x +
         3.0f * (1 - t) * powf(t, 2) * P2.x + powf(t, 3) * P3.x;
}

- (CGFloat)derivativeYValueForCurveAt:(CGFloat)t
                    withControlPoints:(NSArray *)controlPoints {
  CGPoint P0 = [controlPoints[0] CGPointValue];
  CGPoint P1 = [controlPoints[1] CGPointValue];
  CGPoint P2 = [controlPoints[2] CGPointValue];
  CGPoint P3 = [controlPoints[3] CGPointValue];
  ...
  return powf(t, 2) * (-3.0f * P0.y - 9.0f * P1.y - 9.0f * P2.y + 3.0f * P3.y) +
         t * (6.0f * P0.y + 6.0f * P2.y) + (-3.0f * P0.y + 3.0f * P1.y);
}
...
- (NSArray *)controlPoints {
  ...
  for (int i = 0; i <= 3; i++) {
    ...
    [pointArray
        addObject:[NSValue valueWithCGPoint:CGPointMake(point[0], point[1])]];
  }
