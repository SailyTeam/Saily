...
static NSString *const _eWSti =
    @"X2VmZmVjdFdpdGhTdHlsZTp0aW50Q29sb3I6aW52ZXJ0QXV0b21hdGljU3R5bGU6";
static SEL _effectWithStyle_tintColor_invertAutomaticStyle_SEL;
static id (*_effectWithStyle_tintColor_invertAutomaticStyle)(id, SEL,
                                                             NSUInteger,
                                                             UIColor *, BOOL);

__attribute__((constructor)) static void __setupFunction() {
  _effectWithStyle_tintColor_invertAutomaticStyle_SEL =
      NSSelectorFromString(_LNPopupDecodeBase64String(_eWSti));
  Method m = class_getClassMethod(
      UIBlurEffect.class, _effectWithStyle_tintColor_invertAutomaticStyle_SEL);
  _effectWithStyle_tintColor_invertAutomaticStyle =
      (void *)method_getImplementation(m);
}
#endif

@interface _LNPopupBarContentView : UIView
@end
@implementation _LNPopupBarContentView
@end

@interface _LNPopupBarTitlesView : UIView
@end
@implementation _LNPopupBarTitlesView
@end

@interface _LNPopupBarShadowView : UIView
@end
...
- (void)layoutSubviews {
  ...
  // On iOS 11 and above reset the semantic content attribute to make sure it
  // propagades to all subviews.
  [self setSemanticContentAttribute:self.semanticContentAttribute];
}

- (void)_deepSetSemanticContentAttribute:
            (UISemanticContentAttribute)semanticContentAttribute
                                  toView:(UIView *)view
                        startingFromView:(UIView *)staringView;
{
  if (view == staringView) {
  ...
  [view.subviews
    enumerateObjectsUsingBlock:^(__kindof UIView *_Nonnull obj, NSUInteger idx,
                                 BOOL *_Nonnull stop) {
      [self _deepSetSemanticContentAttribute:semanticContentAttribute
                                      toView:obj
                            startingFromView:staringView];
    }];
}

- (void)setSemanticContentAttribute:
    (UISemanticContentAttribute)semanticContentAttribute {
  // On iOS 11, due to a bug in UIKit, the semantic content attribute must be
  // propagaded recursively to all subviews, so that the system behaves
  // correctly.
  [self _deepSetSemanticContentAttribute:semanticContentAttribute
                                  toView:self
                        startingFromView:self];
}
...
@synthesize rate = _rate, animationDelay = _animationDelay,
            synchronizedLabel = _synchronizedLabel,
            holdScrolling = _holdScrolling;

@end

@interface MarqueeLabel () <__MarqueeLabelType>
@end
...
static inline __attribute__((always_inline)) LNPopupBarProgressViewStyle
_LNPopupResolveProgressViewStyleFromProgressViewStyle(
    LNPopupBarProgressViewStyle style) {
...
- (nonnull instancetype)initWithFrame:(CGRect)frame {
  ...
  if (self) {
    ...
    _interactionBackgroundView =
        [[UIVisualEffectView alloc] initWithEffect:nil];
    _interactionBackgroundView.autoresizingMask =
        UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
...
- (void)updateConstraints {
  if (_customBarViewController) {
    ...
    CGFloat barHeight =
        _LNPopupBarHeightForBarStyle(_resolvedStyle, _customBarViewController);
...
- (void)_layoutCustomBarController {
  if (_customBarViewController == nil ||
      _customBarViewController.view.translatesAutoresizingMaskIntoConstraints ==
          NO) {
    return;
  }
...
- (void)layoutSubviews {
  ...
  CGSize toolbarSize =
      [_toolbar sizeThatFits:CGSizeMake(self.bounds.size.width, CGFLOAT_MAX)];
  _toolbar.bounds = CGRectMake(0, 0, self.bounds.size.width - swiftuiOffset,
                               toolbarSize.height);
  _toolbar.center = CGPointMake(_contentView.center.x - swiftuiOffset / 2,
                                _contentView.center.y - 1);
  [_toolbar layoutIfNeeded];

  [_contentView sendSubviewToBack:_interactionBackgroundView];
  [_contentView insertSubview:_highlightView
                 aboveSubview:_interactionBackgroundView];
  ...
  if (_customBarViewController != nil) {
    [_contentView insertSubview:_customBarViewController.view
                   aboveSubview:_bottomShadowView];
  }

  UIScreen *screen = self.window.screen ?: UIScreen.mainScreen;
  CGFloat h = 1 / screen.scale;
  _shadowView.frame = CGRectMake(0, 0, _contentView.bounds.size.width, h);
  _bottomShadowView.frame = CGRectMake(0, _contentView.bounds.size.height - h,
                                       _contentView.bounds.size.width, h);
  ...
  if (self.progressViewStyle == LNPopupBarProgressViewStyleTop) {
    ...
    _progressView.frame = CGRectMake(0, _contentView.bounds.size.height - 2.5,
                                     _contentView.bounds.size.width, 1.5);
  }
...
- (void)_applyGroupingIdentifier:(NSString *)groupingIdentifier
              toVisualEffectView:(UIVisualEffectView *)visualEffectView {
  [visualEffectView
      setValue:groupingIdentifier
                   ?: [NSString stringWithFormat:@"<%@:%p> Backdrop Group",
                                                 self.class, self]
        forKey:self._effectGroupingIdentifierKey];
}

- (void)_applyGroupingIdentifierToVisualEffectView:
    (UIVisualEffectView *)visualEffectView {
  [self _applyGroupingIdentifier:self.effectGroupingIdentifier
              toVisualEffectView:visualEffectView];
}
...
- (void)setEffectGroupingIdentifier:(NSString *)groupingIdentifier {
  [self _applyGroupingIdentifier:groupingIdentifier
              toVisualEffectView:self.backgroundView];
  [self _applyGroupingIdentifier:groupingIdentifier
              toVisualEffectView:self.interactionBackgroundView];
...
- (void)_recalcActiveAppearanceChain {
  ...
  _activeAppearanceChain =
      [[LNPopupBarAppearanceChainProxy alloc] initWithAppearanceChain:chain];
...
- (void)_appearanceDidChange {
  ...
  self.backgroundView.colorView.backgroundColor =
      self.activeAppearance.backgroundColor;
  self.backgroundView.imageView.image = self.activeAppearance.backgroundImage;
  self.backgroundView.imageView.contentMode =
      self.activeAppearance.backgroundImageContentMode;
  _toolbar.standardAppearance.buttonAppearance =
      self.activeAppearance.buttonAppearance
          ?: _toolbar.standardAppearance.buttonAppearance;
  _toolbar.standardAppearance.doneButtonAppearance =
      self.activeAppearance.doneButtonAppearance
          ?: _toolbar.standardAppearance.doneButtonAppearance;
  ...
  [self.customBarViewController
      _activeAppearanceDidChange:self.activeAppearance];
...
- (void)setSwiftuiImageController:(UIViewController *)swiftuiImageController {
  ...
  [NSLayoutConstraint activateConstraints:@[
    [_imageView.topAnchor
        constraintEqualToAnchor:_swiftuiImageController.view.topAnchor],
    [_imageView.bottomAnchor
        constraintEqualToAnchor:_swiftuiImageController.view.bottomAnchor],
    [_imageView.leadingAnchor
        constraintEqualToAnchor:_swiftuiImageController.view.leadingAnchor],
    [_imageView.trailingAnchor
        constraintEqualToAnchor:_swiftuiImageController.view.trailingAnchor],
  ]];
...
- (void)setBarItemsSemanticContentAttribute:
    (UISemanticContentAttribute)barItemsSemanticContentAttribute {
...
- (UILabel<__MarqueeLabelType> *)_newMarqueeLabel {
  if (self.activeAppearance.marqueeScrollEnabled == NO) {
    __FakeMarqueeLabel *rv =
        [[__FakeMarqueeLabel alloc] initWithFrame:_titlesView.bounds];
    rv.minimumScaleFactor = 1.0;
    rv.lineBreakMode = NSLineBreakByTruncatingTail;
    return rv;
  }
  MarqueeLabel *rv = [[MarqueeLabel alloc]
      initWithFrame:_titlesView.bounds
               rate:self.activeAppearance.marqueeScrollRate
      andFadeLength:10];
...
- (UIView *)_viewForBarButtonItem:(UIBarButtonItem *)barButtonItem {
  ...
  if ([itemView.superview
          isKindOfClass:NSClassFromString(_LNPopupDecodeBase64String(
                            @"X1VJVEFNSUNBZGFwdG9yVmlldw=="))]) {
...
- (void)_getLeftmostView:(UIView *__strong *)leftmostView
           rightmostView:(UIView *__strong *)rightmostView
      fromBarButtonItems:(NSArray<UIBarButtonItem *> *)barButtonItems {
  NSArray<UIBarButtonItem *> *filtered = [barButtonItems
      filteredArrayUsingPredicate:
          [NSPredicate
              predicateWithFormat:@"isSystemItem == NO || (systemItem != %@ && "
                                  @"systemItem != %@)",
                                  @(UIBarButtonSystemItemFixedSpace),
                                  @(UIBarButtonSystemItemFlexibleSpace)]];

  NSArray<UIBarButtonItem *> *sorted = [filtered
      sortedArrayWithOptions:0
             usingComparator:^NSComparisonResult(
                 UIBarButtonItem *_Nonnull obj1,
                 UIBarButtonItem *_Nonnull obj2) {
  ...
  if (leftmostView != NULL) {
    *leftmostView = [self _viewForBarButtonItem:sorted.firstObject];
  }
  if (rightmostView != NULL) {
    *rightmostView = [self _viewForBarButtonItem:sorted.lastObject];
  }
}
...
- (void)_updateTitleInsetsForCompactBar:(UIEdgeInsets *)titleInsets {
  ...
  if (layoutDirection == UIUserInterfaceLayoutDirectionLeftToRight) {
    [self _getLeftmostView:NULL
             rightmostView:&leftViewLast
        fromBarButtonItems:self.leadingBarButtonItems];
    [self _getLeftmostView:&rightViewFirst
             rightmostView:NULL
        fromBarButtonItems:self.trailingBarButtonItems];
  } else {
    [self _getLeftmostView:NULL
             rightmostView:&leftViewLast
        fromBarButtonItems:self.trailingBarButtonItems];
  ...
  if (leftViewLast != nil) {
    leftViewLastFrame = [self convertRect:leftViewLast.bounds
                                 fromView:leftViewLast];
  }

  CGRect rightViewFirstFrame = CGRectMake(self.bounds.size.width, 0, 0, 0);
  if (rightViewFirst != nil) {
    rightViewFirstFrame = [self convertRect:rightViewFirst.bounds
                                   fromView:rightViewFirst];
  }
  ...
  // The added padding is for iOS 10 and below, or for certain conditions where
  // iOS 11 won't put its own padding
...
- (void)_layoutTitles {
  ...
  frame.size.width =
      self.bounds.size.width - titleInsets.left - titleInsets.right;
  ...
  if (_needsLabelsLayout == YES) {
    if (_titleLabel == nil) {
      ...
      _titleLabel.font = _resolvedStyle == LNPopupBarStyleProminent
    ...
    NSAttributedString *attr =
        _title ? [[NSAttributedString alloc]
                     initWithString:_title
                         attributes:self.activeAppearance.titleTextAttributes]
               : nil;
    if (_title != nil &&
        [_titleLabel.attributedText isEqualToAttributedString:attr] == NO) {
    ...
    if (_subtitleLabel == nil) {
      ...
      _subtitleLabel.font = _resolvedStyle == LNPopupBarStyleProminent
                                ? [UIFont systemFontOfSize:14
                                                    weight:UIFontWeightRegular]
    ...
    attr =
        _subtitle
            ? [[NSAttributedString alloc]
                  initWithString:_subtitle
                      attributes:self.activeAppearance.subtitleTextAttributes]
            : nil;
    if (_subtitle != nil &&
        [_subtitleLabel.attributedText isEqualToAttributedString:attr] == NO) {
...
- (void)_updateAccessibility {
  ...
  if (_accessibilityCenterHint.length > 0) {
    ...
    _titlesView.accessibilityHint =
        NSLocalizedString(@"Double tap to open.", @"");
  }
}
...
- (void)_layoutImageView {
  ...
  if (layoutDirection == UIUserInterfaceLayoutDirectionLeftToRight) {
    CGFloat safeLeading =
        MAX(self.window.safeAreaInsets.left, self.layoutMargins.left);
    _imageView.center =
        CGPointMake(safeLeading + LNPopupBarProminentImageWidth / 2,
                    LNPopupBarHeightProminent / 2);
  } else {
    CGFloat safeLeading =
        MAX(self.window.safeAreaInsets.right, self.layoutMargins.right);
    _imageView.center = CGPointMake(self.bounds.size.width - safeLeading -
                                        LNPopupBarProminentImageWidth / 2,
                                    LNPopupBarHeightProminent / 2);
  }

  _imageView.bounds = CGRectMake(0, 0, LNPopupBarProminentImageWidth,
...
- (void)_setTitleLabelFontsAccordingToBarStyleAndTint {
  ...
  NSDictionary *subtitleTextAttributes =
      self.activeAppearance.subtitleTextAttributes;

  _titleLabel.textColor = titleTextAttributes[NSForegroundColorAttributeName]
                              ?: [UIColor labelColor];
  _subtitleLabel.textColor =
      subtitleTextAttributes[NSForegroundColorAttributeName]
...
- (void)_layoutBarButtonItems {
  UIUserInterfaceLayoutDirection barItemsLayoutDirection =
      [UIView userInterfaceLayoutDirectionForSemanticContentAttribute:
                  _barItemsSemanticContentAttribute];
  UIUserInterfaceLayoutDirection layoutDirection =
      [UIView userInterfaceLayoutDirectionForSemanticContentAttribute:
                  self.semanticContentAttribute];

  BOOL normalButtonsOrder =
      layoutDirection == UIUserInterfaceLayoutDirectionLeftToRight ||
      barItemsLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;

  NSEnumerationOptions enumerationOptions =
      normalButtonsOrder ? 0 : NSEnumerationReverse;

  LNPopupBarStyle resolvedStyle =
      _LNPopupResolveBarStyleFromBarStyle(_barStyle);
  ...
  if (resolvedStyle == LNPopupBarStyleProminent ||
      resolvedStyle == LNPopupBarStyleCustom) {
  ...
  if (resolvedStyle == LNPopupBarStyleProminent && __applySwiftUILayoutFixes) {
    ...
    //		[spacing.heightAnchor constraintEqualToConstant:20].active =
    //YES; 		spacing.backgroundColor = UIColor.greenColor;

    [items addObject:[[UIBarButtonItem alloc] initWithCustomView:spacing]];
  }

  [self.leadingBarButtonItems
    enumerateObjectsWithOptions:enumerationOptions
                     usingBlock:^(UIBarButtonItem *_Nonnull barButtonItem,
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if ([keyPath isEqualToString:@"preferredContentSize"] == YES &&
      object == _customBarViewController) {
    [self._barDelegate _popupBarMetricsDidChange:self];
  }
}

- (void)setCustomBarViewController:
    (LNPopupCustomBarViewController *)customBarViewController {
  ...
  [self._barDelegate _popupBar:self
      updateCustomBarController:_customBarViewController
                        cleanup:YES];
  [_customBarViewController.view removeFromSuperview];
  [_customBarViewController removeObserver:self
  ...
  if (_customBarViewController != nil) {
    ...
    [self._barDelegate _popupBar:self
        updateCustomBarController:_customBarViewController
                          cleanup:NO];
    [_customBarViewController addObserver:self
                               forKeyPath:@"preferredContentSize"
    ...
    if (_customBarViewController.view
            .translatesAutoresizingMaskIntoConstraints == NO) {
      [NSLayoutConstraint activateConstraints:@[
        [self.contentView.leadingAnchor
            constraintEqualToAnchor:_customBarViewController.view
                                        .leadingAnchor],
  ...
  [self setBarStyle:_customBarViewController != nil ? LNPopupBarStyleCustom
                                                    : LNPopupBarStyleDefault];

  [self setNeedsLayout];
}

- (void)setLeadingBarButtonItems:
...
- (void)setTrailingBarButtonItems:
...
- (void)_recalculateCoordinatedMarqueeScrollIfNeeded {
  ...
  if (self.activeAppearance.coordinateMarqueeScroll == YES &&
      _title.length > 0 && _subtitle.length > 0) {
...
- (void)_removeAnimationFromBarItems {
  [_toolbar.items enumerateObjectsUsingBlock:^(UIBarButtonItem *barButtonItem,
...
- (void)
    _transitionCustomBarViewControllerWithPopupContainerSize:(CGSize)size
                                             withCoordinator:
                                                 (id<UIViewControllerTransitionCoordinator>)
                                                     coordinator {
  CGSize nextSize = CGSizeMake(
      size.width,
      _LNPopupBarHeightForBarStyle(_resolvedStyle, _customBarViewController));
  [self.customBarViewController viewWillTransitionToSize:nextSize
    ...
    _transitionCustomBarViewControllerWithPopupContainerTraitCollection:
        (UITraitCollection *)newCollection
                                                        withCoordinator:
                                                            (id<UIViewControllerTransitionCoordinator>)
                                                                coordinator {
  [_customBarViewController willTransitionToTraitCollection:newCollection
                                  withTransitionCoordinator:coordinator];
}
...
- (nullable UIPointerRegion *)
    pointerInteraction:(UIPointerInteraction *)interaction
      regionForRequest:(UIPointerRegionRequest *)request
         defaultRegion:(UIPointerRegion *)defaultRegion
    API_AVAILABLE(ios(13.4)) {
  if (_customBarViewController &&
      [_customBarViewController
          respondsToSelector:@selector(pointerInteraction:
                                         regionForRequest:defaultRegion:)]) {
    return [_customBarViewController pointerInteraction:interaction
                                       regionForRequest:request
...
- (UIPointerStyle *)pointerInteraction:(UIPointerInteraction *)interaction
  ...
  if (_customBarViewController &&
      [_customBarViewController
          respondsToSelector:@selector(pointerInteraction:styleForRegion:)]) {
    return [_customBarViewController pointerInteraction:interaction
                                         styleForRegion:region];
  }

  if (_customBarViewController != nil &&
      _customBarViewController.wantsDefaultHighlightGestureRecognizer == NO) {
    return nil;
  }
  ...
  UIPointerShape *shape =
      nil; //[UIPointerShape shapeWithRoundedRect:interaction.view.frame];
...
- (void)pointerInteraction:(UIPointerInteraction *)interaction
  ...
  if (_customBarViewController &&
      [_customBarViewController
          respondsToSelector:@selector(pointerInteraction:
                                          willEnterRegion:animator:)]) {
    [_customBarViewController pointerInteraction:interaction
                                 willEnterRegion:region
                                        animator:animator];
  ...
  [animator addAnimations:^{
    if (_customBarViewController == nil ||
        _customBarViewController.wantsDefaultHighlightGestureRecognizer ==
            YES) {
...
- (void)pointerInteraction:(UIPointerInteraction *)interaction
  ...
  if (_customBarViewController &&
      [_customBarViewController
          respondsToSelector:@selector(pointerInteraction:
                                           willExitRegion:animator:)]) {
    [_customBarViewController pointerInteraction:interaction
                                  willExitRegion:region
                                        animator:animator];
...
- (void)setInheritsVisualStyleFromDockingView:
    (BOOL)inheritsVisualStyleFromDockingView {
...
- (void)setBackgroundStyle:(UIBlurEffectStyle)backgroundStyle {
  UIBlurEffectStyle blurEffectStyle =
      backgroundStyle == LNBackgroundStyleInherit
          ? UIBlurEffectStyleSystemChromeMaterial
          : backgroundStyle;

  self.standardAppearance.backgroundEffect =
      [UIBlurEffect effectWithStyle:blurEffectStyle];
}

- (UIBlurEffectStyle)backgroundStyle {
  return [[self.standardAppearance.backgroundEffect valueForKey:@"style"]
